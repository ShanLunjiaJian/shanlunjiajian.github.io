...
inline void double_dft(int n,int *F,int *TF0,int *TF){ /*todo*/ }
inline void exp(int n,int *_F,int *G)//18E(n)
{
	static int F[N],T[N],H[N],H0[N],G0[N],TH0[N],TH[N],TG0[N],TG00[N],FP[N],TFP[N];
	memcpy(F,_F,sizeof(int)*n),memcpy(FP,F,sizeof(int)*n),deriv(n,F);
	G[0]=1,H[0]=1;
	for(int l=2,mid=1;(l<<1)<=n;l<<=1,mid<<=1)
	{
		memcpy(H0,H,sizeof(int)*mid),memcpy(G0,G,sizeof(int)*l);
		
		memcpy(TH0,H0,sizeof(int)*mid),memcpy(TG0,G0,sizeof(int)*l);
		dft(l<<1,TH0),dft(l<<1,TG0);
		for(int i=0;i<(l<<1);i++) H[i]=(long long)TH0[i]*TG0[i]%P;
		idft(l<<1,H),memset(H,0,sizeof(int)*mid);
		for(int i=0;i<l;i++) mod_(H[i]=(H0[i]<<1)-H[i]),mod(H[i]);
		
		memcpy(TG00,TG0,sizeof(int)*l);
		memcpy(TFP,F,sizeof(int)*l),deriv(l,TFP),dft(l,TFP);
		for(int i=0;i<l;i++) T[i]=(long long)TG00[i]*TFP[i]%P;
		idft(l,T),memcpy(T+mid,T,sizeof(int)*mid),memset(T,0,sizeof(int)*mid);
		
		memcpy(TH,H,sizeof(int)*l),dft(l<<1,TH);
		dft(l<<1,T);
		for(int i=0;i<(l<<1);i++) T[i]=(long long)T[i]*TH[i]%P;
		idft(l<<1,T),memcpy(T+l,T,sizeof(int)*l),memset(T,0,sizeof(int)*l);
		
		for(int i=0;i<(l<<1);i++) T[i]=T[i]+FP[i];
		integ(l<<1,T);
		for(int i=0;i<(l<<1);i++) mod(T[i]=F[i]+P-T[i]);
		
		dft(l<<1,T);
		for(int i=0;i<(l<<1);i++) T[i]=(long long)T[i]*TG0[i]%P;
		idft(l<<1,T),memcpy(T+mid,T,sizeof(int)*mid),memset(T,0,sizeof(int)*mid);
