...
有一个简单方法可以做到$$16.5\mathrm{E}(n)$$。

![img](/img/2021-10-25-board/exp.png)

呃他在说什么?我们来解释一下。

首先还需要一点前置知识，也就是如果我们知道了$$\mathcal{F}_n$$，计算$$\mathcal{F}_{2n}$$只需要$$E(n)$$的时间，因为dft实际上是一组点值。我们考虑少了的那一部分，实际上只需要先代入一次本原单位根$$\omega_{2n}$$更新系数，然后做dft即可。最后把两边做一层dft合并起来。
