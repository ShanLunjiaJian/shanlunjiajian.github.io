...
long long dfs(int u,long long f,int t)
{
	...
	{
		cur[u]=i;
		if(dis[e[i].v]==dis[u]+1){ temp=dfs(e[i].v,min(f,e[i].f),t); sum+=temp,f-=temp; e[i].f-=temp,e[i^1].f+=temp; if(!f) return sum; }
	}
	return sum;
}
inline long long dinic(int s,int t){ long long ans=0; while(bfs(s)) ans+=dfs(s,0x3f3f3f3f3f3f3f3fll,t); return ans; }
...
inline void sieve(int n)
{
	...
	for(int i=2;i<=n;i++)
	{
		if(!b[i]) prime[++pcnt]=i,minp[i]=i,cnt[i]=1,d[i]=2,mu[i]=-1,phi[i]=i-1;
		for(int j=1;j<=pcnt&&i*prime[j]<=n;j++)
		{
			b[i*prime[j]]=1,minp[i*prime[j]]=prime[j];
			if(!(i%prime[j]))
			{
				mu[i*prime[j]]=0;
...
```cpp
#define T int
inline void radix_sort(int n,T *a)
{
...
{\
    memset(b,0,sizeof(b));\
    for (T *it=a+n;it!=a;it--) ++b[(it[-1]>>B)&255];\
    for (unsigned int *it=b;it!=b+255;it++) it[1]+=it[0];\
    for (T *it=a+n;it!=a;it--) t[--b[(it[-1]>>B)&255]]=it[-1];\
}
...
struct RandI64{ unsigned long long x; inline RandI64(unsigned long long _x=88172645463325252ll):x(_x){} inline unsigned long long operator (){ return x^=x<<13,x^=x>>7,x^=x<<17; } };
