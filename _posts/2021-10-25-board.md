...
```cpp
void tle_assert(){ int c=0; while(1) c++; printf("%d\n",c); }
void mle_assert(){ new a[1000000000]; }
...
for xx open cup, gp of japan J. Median Replace Hard.
...
struct NFA
{
    vector<array<vector<int>,Sigma> > e;
    vector<int> type;
    vector<int> start;
    int size(){ return e.size(); }
    void resize(int n){ e.resize(n),type.resize(n); }
    int add_node(int _type=0){ e.emplace_back(),type.push_back(_type); return e.size()-1; }
    array<vector<int>,Sigma>& operator [] (int i){ return e[i]; }
    ...
    NFA reverse()
    {
        NFA r;
        r.resize(e.size());
        r.start.resize(e.size());
        int cnt=0;
        for(int i=0;i<e.size();i++) if(type[i]) r.start[i]=1;
        r.type[0]=1;
        for(int i=0;i<e.size();i++)
            for(int c=0;c<Sigma;c++)
                for(int j:e[i][c]) r[j][c].push_back(i);
        return r;
    }
    void output()
    {
        printf("nfa output start:\n");
        for(int i=0;i<e.size();i++)
            printf("type of %d: %d\n",i,type[i]);
        for(int i=0;i<e.size();i++)
            for(int c=0;c<Sigma;c++)
...
struct DFA
{
    vector<array<int,Sigma> > e;//edges
    //starting state is 0
    vector<int> type;
    //type of each node. you can set 0=refuse, 1=accept, etc.
    //types should not be too large
    int size(){ return e.size(); }
    void resize(int n){ e.resize(n),type.resize(n); }
    int add_node(int _type=0){ e.push_back(array<int,Sigma>()),type.push_back(_type); return e.size()-1; }
    array<int,Sigma>& operator [] (int i){ return e[i]; }
    ...
    operator NFA()
    {
        ...
        r.type=type;
        for(int i=0;i<e.size();i++)
            for(int c=0;c<Sigma;c++) r[i][c].push_back(e[i][c]);
        return r;
    }

    DFA minimize()//generate a minimized dfa
    {
        //remove useless states
        vector<bool> vis;
        vis.resize(e.size());
        function<void(int)> dfs;
        dfs=[&](int u){ vis[u]=1; for(int c=0;c<Sigma;c++) if(!vis[e[u][c]]) dfs(e[u][c]); };
        dfs(0);

        //calculate equivalent classes
        vector<int> belong;
        vector<vector<int> > ec,in;
        //(e)quivalent (c)lass
        belong.resize(e.size());
        int max_type=0;
        for(int i=0;i<e.size();i++) if(vis[i]) max_type=max(type[i],max_type);
        ec.resize(max_type+1);
        for(int i=0;i<e.size();i++) if(vis[i]) ec[type[i]].push_back(i);
        for(int cnt=0;;cnt++)
        {
            int last_size=ec.size();
            for(int c=0;c<Sigma;c++)
            {
                in.resize(ec.size());
                for(int i=0;i<ec.size();i++)
                    for(int u:ec[i]) belong[u]=i;
                vector<vector<int> > ec_new;
                for(vector<int> cur:ec)
                {
                    int sum=0;
                    for(int u:cur) in[belong[e[u][c]]].push_back(u);
                    for(int u:cur) if(!in[belong[e[u][c]]].empty())
                        sum+=in[belong[e[u][c]]].size(),
                        ec_new.push_back(in[belong[e[u][c]]]),
                        vector<int>().swap(in[belong[e[u][c]]]);
                }
                ec.swap(ec_new);
            }
            if(ec.size()==last_size) break;
        }
        //generate new dfa
        DFA r;
        r.resize(ec.size());
        for(int i=0;i<ec.size();i++)
        {
            const int u=ec[i][0];
            r.type[i]=type[u];
            for(int c=0;c<Sigma;c++) r[i][c]=belong[e[u][c]];
        }
        int b=0;
        for(int u:ec[0]) b|=(u==0);
        assert(b);
        return r;
    }

    DFA substitute(vector<DFA> d)
    {
        NFA r=*this;
        r.resize(e.size());
        r.type=type;
        ...
        eps.resize(e.size());
        for(int i=0;i<e.size();i++)
            for(int c=0;c<Sigma;c++) eps[i].push_back(r.add_node());
        for(int i=0;i<e.size();i++)
            for(int c=0;c<Sigma;c++)
            {
                for(int f=0;f<Sigma;f++) r[i][c].push_back(eps[e[i][c]][f]);
                vector<int> node;
                node.push_back(eps[i][c]);
                for(int j=1;j<d[c].size();j++) node.push_back(r.add_node());
                for(int j=0;j<d[c].size();j++)
                    for(int f=0;f<Sigma;f++)
                    {
                        r[node[j]][f].push_back(node[d[c][j][f]]);
                        if(d[c].type[d[c][j][f]])
                        {
                            r[node[j]][f].push_back(e[i][c]);
                            for(int g=0;g<Sigma;g++) r[node[j]][f].push_back(eps[e[i][c]][g]);
                        }
                    }
            }
        r.start.resize(r.e.size());
        r.start[0]=1;
        for(int i=0;i<Sigma;i++) r.start[eps[0][i]]=1;
        return ((NFA)(r.reverse().determinize())).reverse().determinize();
    }

    void output()
    {
        printf("dfa output start:\n");
        for(int i=0;i<e.size();i++)
            printf("type of %d: %d\n",i,type[i]);
        for(int i=0;i<e.size();i++)
        {
            if(e[i][0]==e[i][1]) printf("%d %d *\n",i,e[i][0]);
            else printf("%d %d %d\n%d %d %d\n",i,e[i][0],0,i,e[i][1],1);
        }
        printf("output end\n");
    }
...
DFA NFA::determinize()
{
    ...
    dfs=[&dfs,&r,&mp,this](vector<int> state)->int
    {
        if(mp.find(state)!=mp.end()) return mp[state];
        bool t=0;
        for(int i=0;i<e.size();i++) if(state[i]&&type[i]) t=1;
        int u;
        mp[state]=u=r.add_node(t);
        for(int c=0;c<Sigma;c++)
        {
            vector<int> new_state(e.size());
            for(int i=0;i<e.size();i++) if(state[i])
                for(int v:e[i][c]) new_state[v]=1;
            r[u][c]=dfs(new_state);
        }
        return u;
    };
    vector<int> S;
    if(!start.empty()) S=start;
    else S.resize(e.size()),S[0]=1;
    dfs(S);
    return r;
}

#define dfa_product(a,b,op)\
DFA r;\
r.resize(a.size()*b.size());\
for(int i=0;i<a.size();i++)\
    for(int j=0;j<b.size();j++)\
    {\
        r.type[i*b.size()+j]=(a.type[i] op b.type[j]);\
        for(int c=0;c<Sigma;c++)\
            r[i*b.size()+j][c]=a[i][c]*b.size()+b[j][c];\
    }\
return r;

DFA operator + (DFA a,DFA b){ dfa_product(a,b,|); }
DFA operator * (DFA a,DFA b)
{
    ...
    for(int i=0;i<a.size();i++)
        for(int c=0;c<Sigma;c++)
        {
            r[i][c].push_back(a[i][c]);
            if(a.type[a[i][c]]) r[i][c].push_back(a.size());
        }
    for(int i=0;i<b.size();i++)
        for(int c=0;c<Sigma;c++) r[i+a.size()][c].push_back(b[i][c]+a.size());
    for(int i=0;i<b.size();i++) r.type[i+a.size()]=b.type[i];
    return ((NFA)(r.reverse().determinize())).reverse().determinize();
}

bool operator == (DFA a,DFA b)
{
    if(a.size()!=b.size()) return 0;
    function <bool(int,int)> dfs;
    vector<int> mpa,mpb;
    mpa.resize(a.size()),mpb.resize(b.size());
    for(int i=0;i<a.size();i++) mpa[i]=mpb[i]=-1;
    dfs=[&](int u,int v)->bool
    {
        if(a.type[u]!=b.type[v]) return 0;
        if(mpa[u]==-1&&mpb[v]==-1) mpa[u]=v,mpb[v]=u;
        else if(mpa[u]==v&&mpb[v]==u) return 1;
        else return 0;
        bool flag=1;
        for(int c=0;c<Sigma;c++)
            flag&=dfs(a[u][c],b[v][c]);
        return flag;
    };
    return dfs(0,0);
}

inline unsigned long long randi(){ static unsigned long long x=34145131426564ll; return x^=x<<13,x^=x>>7,x^=x<<17; }
...
int main()
{
    ...
    while(T--)
    {
        ...
        d[0][1]={3,4};//0
        d[0][2]={5,6};//1
        d[0][3]={P[0]+1,P[4]+1};//00
        d[0][4]={P[2]+1,P[6]+1};//01
        d[0][5]={P[1]+1,P[5]+1};//10
        d[0][6]={P[3]+1,P[7]+1};//11

        d[1]=d[0];
        d[0].type[1]=1,d[1].type[2]=1;

        d[0]=d[0].minimize(),d[1]=d[1].minimize();

        vector<DFA> nd=d;

        for(int T=1;d[0].size()<100;T++)
        {
            vector<DFA> last_d=d;
            d={d[0].substitute(nd),d[1].substitute(nd)};
            if(last_d[0]==d[0]&&last_d[1]==d[1]) break;
        }
        ...
        DFA t[2];
        t[0].resize(3);
        t[0][0]={1,2},t[0][1]={2,2},t[0][2]={2,2},t[0].type={0,1,0};
        t[1]=t[0];
        t[1][0]={2,1};
        for(int i=0;i<8;i++)
            t[P[i]]=(t[P[i]]+d[i&1]*d[(i>>1)&1]*d[(i>>2)&1]).minimize();
        assert(t[0]==d[0]&&t[1]==d[1]);

        //dp

        int n=strlen(s);
        memset(dp,0,sizeof(int)*(n+1)*40);
        dp[0][0]=1;
        for(int i=0;i<n;i++)
        {
            if(s[i]!='1')
                for(int j=0;j<d[1].size();j++)
                    mod(dp[i+1][d[1][j][0]]+=dp[i][j]);
            if(s[i]!='0')
                for(int j=0;j<d[1].size();j++)
                    mod(dp[i+1][d[1][j][1]]+=dp[i][j]);
        }
        int ans=0;
        for(int j=0;j<d[1].size();j++) if(d[1].type[j])
            mod(ans+=dp[n][j]);
        printf("%d\n",ans);
    }
    return 0;
...
```cpp
struct istream{
    static const int L=1<<26;
    char buf[L],*s;
    inline istream(){ s=buf+L; }
    inline char get(){ if(s==buf+L) fread(s=buf,1,1<<26,stdin); return *s++; }
...
inline unsigned long long randi(){ static unsigned long long x=ll; return x^=x<<13,x^=x>>7,x^=x<<17; }
