...
考虑另一个简单结论 : 如果我执行了两次$$-1$$，再执行一次$$\mathrm{shift}$$，等价于先执行一次$$\mathrm{shift}$$，再执行一次$$-1$$。这说明除了最后一段-1的数量可能很多，前面的$$-1$$都最多只有一个。

反过来，这也说明，每个$$-1$$都可以向前退位，于是除了第一段和最后一段有若干个$$-1$$，别的地方都可以没有$$-1$$。
...
显然可以限制$$a<2^k$$，否则$$\lfloor\frac{a}{2^k}\rfloor$$的部分可以直接放进$$b$$里。

于是$$\lfloor\frac{x-a}{2^k}\rfloor$$的值只可能是$$\lfloor\frac{x}{2^k}\rfloor$$或$$\lfloor\frac{x}{2^k}\rfloor-1$$，这取决于$$x-a$$的时候有没有在$$k$$位上产生退位。

于是每个数都在一个$$a$$的前缀里得到$$\lfloor\frac{x}{2^k}\rfloor$$，在对应的后缀里得到$$\lfloor\frac{x}{2^k}\rfloor-1$$，这$$n$$个数合并得到一个$$O(n)$$段的分段函数，每一段表示一种可能的差分序列和它的$$\min$$的可行区间。

枚举$$k$$，对这总共$$O(n\log v)$$个差分序列去重，每个等价类做一个区间并，然后直接计算答案即可。
...
ARC097F : 可以简单地构造出$$2n$$的方案，不过这没什么用（
...
\* : 语出松毛虫《无题》 : 无聊的水一天群 不过是一群人 发着smc! 但当我用上了换行 我仿佛，成了诗人
...
但是它是$$O(n\log n)$$的!瓶颈在于逆序对，注意到这个是膜$$3$$的，所以考虑一个牛逼数据结构。我们令$$v=2^32$$也就是uint的范围，按$$\lfloor\log_3 v\rfloor=\Theta(\log v)$$大小分块(这个数实际上是$$20$$)，整块用BiT维护是$$O(1+\log n/\log w)=O(1)$$；零散块压成一个uint，给值域开根号地打表来支持求后缀和(，具体地就是打表$$f(S,i)$$表示$$S$$表示的块状态的后缀$$i$$的和，只需要打到$$3^10$$)。这个应该比块长$$\log n$$块一些?我不知道，反正不会去写（
