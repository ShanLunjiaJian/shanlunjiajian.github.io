...
这个题还是有点牛逼了，我可能很缺少感觉。结论是，一个方案可行，当且仅当它有周期$$\gcd(h,w)$$，并且在每个周期内，设它有$$d,r$$个向下、向右移动，则$$\gcd(d,h)=\gcd(r,w)=1$$。所以我们直接枚举$$d$$统计答案。
...
这个好像也是板子。如果转移的这一段没有一个非平凡整周期，那么可以前缀和算，而有非平凡整周期的转移可以用$$O(n\log n)$$个等差数列表示，这些等差数列可以用runs算出来。由于所有runs

E. Edit

dp。设$$dp(u,v,i,j)$$表示希望把$$u$$子树操作成$$v$$子树，目前已经用$$u$$的前$$i$$个儿子搞出$$v$$的前$$j$$个儿子的最小代价，$$s(u)$$表示从空开始搞出$$u$$子树的代价。$$(v,j)$$的对数是$$O(n_2)$$，所以复杂度是$$O(n_1^2n_2)$$这样的。
