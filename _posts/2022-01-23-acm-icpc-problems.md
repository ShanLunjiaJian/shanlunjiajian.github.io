...
为了大概地证明这个结论，猜测对于任意策略，答案$$f(x)$$大概是超鞅(期望比当前值小)，首先它肯定不是鞅，然后如果它是亚鞅，我们不停赌当前值的一半就赢了。超鞅也就是说$$f(x+k)p+f(x-k)q\leq f(x)$$，其中$$q=1-p$$。感觉上如果$$f$$能够全部取得等号，就无敌了，打表发现那个策略真的取到了，所以就赢麻了。
...
google一下，你会发现这个东西称作right stochastic matrix 马尔可夫矩阵。我们所要求的答案$$x$$满足$$Ax=x$$，所以问题是求特征向量。计算其特征多项式，
