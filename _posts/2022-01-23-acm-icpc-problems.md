...
类似于unr2022 D1B，我们贪心匹配，在匹配不上的时候倒回去。二分倒到哪即可。
...
但是不是。考虑扫值域，那么也就是每个集合是值域上的一个区间，那么相当于给定若干个区间中的所有元素(但是不知道区间的位置)，构造一个方案，这个看起来更为好做了。

考虑所有不被任何区间包含的区间，那么最后的方案中其中必有一个最左的，它满足和剩下的不被包含的区间的交从小到大排序之后，得到一条包含链，而不是两条，而显然满足后面这个条件的也一定可以当这个最左的。这些链把序列分成若干段，每一段里面的元素都确定了。