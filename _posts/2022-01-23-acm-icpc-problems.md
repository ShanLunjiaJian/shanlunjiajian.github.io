...
经过至少一次排序的用一个trie维护，刚插入还没经过排序的用一个线段树维护，然后每次排序之后把线段树全部插进trie。每次xor在线段树上是简单的，在trie上表现为打了一个子树中若干层交换儿子的标记，然后在查询的时候如果这个标记之后进行了排序那么儿子就已经被换过来了，否则就没有被换。总复杂度$$O(n\log^2 v)$$。
...
当场无人通过，问了切队长和唐队长好像也没有什么牛逼搜法。不知道正解是什么东西。
