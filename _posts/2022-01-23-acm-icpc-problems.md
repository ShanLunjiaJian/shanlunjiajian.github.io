...
然后另一个简单想法是法法塔求出每个距离有多少对点。想到这个之后问题突然变得简单了，因为一共有$$\frac{n(n-1)}{2}$$对点，距离都在$$2m$$以内，所以如果$$\frac{n(n-1)}{2}>2m$$必然有解，我们只需要保留前面$$O(\sqtr{m})$$个爆力就行了。
...
优化一下，注意到如果$$w$$很小我们可以直接这么做，如果$$w$$达到了$$\log$$级别，那么实际上根本不会把$$w$$用完，直接搞就行了。所以复杂度只有$$O(k\log^2 k)$$。
...
坐标范围是$$v$$的话，任意两个点的极角($$\frac{\mathrm{atan2}(x,y)}{\pi}$$)差不超过$$\frac{1}{v}$$级别，于是我们只需要比较到$$\frac{1}{v}$$级别，而$$v=n^{O(1)}$$。
...
hash一下，或者排个序(实际上好像排序更简单，直接拿个trie往里插就行了)，然后变成强制在线区间数颜色，主席树即可。
