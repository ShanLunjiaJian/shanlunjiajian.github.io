...
容易考虑枚举长度$$l$$然后根号分治，主要问题是$$l$$很大怎么做。考虑此时每一段都很长，那么设$$p(i)=[s_i=s_{i+l}]$$，如果$$p$$中有两个很近的$$0$$，看起来就有很多区间没有了。如果两个$$0$$很远，这样的$$0$$个数很少。考虑了各种想法之后，发现了一个较为优美的做法，我们维护还没确定能否转移的部分的左端点$$p$$和$$p$$右边第一个$$0$$的位置$$i$$，那么$$i-p\leq l$$，一开始$$p=1$$。找到$$[p,p+L-1]$$中最右的$$0$$
...
考虑我们先问所有点，那么有一些凹的部分需要砍掉。我们枚举每个点删掉它，看它是不是在凸包上，如果不是那么就不是了，然后我们会得到一些凹的区间，对每个区间递归下去处理，这些区间构成的多边形可能还是凹的，所以我们要继续递归。
