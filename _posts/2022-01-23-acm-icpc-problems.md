...
发现这个东西看起来有点积性。考虑两个互素的数的方案，发现如果不要求非$$1$$，我们直接乘起来就好了，而要求非$$1$$的话，猜测可以钦点一些$$1$$来容斥。每个素因数是一个组合数，算即可。

考虑最后的容斥，看起来这玩意好像就是一个二项式反演。结束了。
...
类似 zjoi2017 多项式。$$n-1$$次多项式$$f$$，查询$$f^m$$的一个区间中，给定的长$$k$$的串$$t$$出现多少次。$$n\leq k=18,m\leq 10^{16}$$。
...
所以看起来难度瓶颈是平方并乘$$f$$。考虑我们直接维护每种长$$k$$的子串的出现次数，那么发现平方之后每个子串长度都变成了$$2k-1$$，而乘最多$$k-1$$次的$$f$$之后$$k-1,...,2k-2$$的值必然还是正确的，所以只需要特殊处理最低的和最高的若干个子串。