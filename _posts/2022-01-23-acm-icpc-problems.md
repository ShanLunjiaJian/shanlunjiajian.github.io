...
dij。比较两个状态，只需要以回合数为第一关键字，剩余的体力为第二关键字。这个也告诉我们，只要距离是有序的，并且走一步不会让距离变小，就可以使用dij。
...
考虑$$l$$怎么算。重设$$l(i,j)$$表示前缀$$i$$中已经有了$$j$$个向左飞出去的手的，于是还需要$$b-j$$个的概率，那么如果遇到一个花，就需要后面的手把它干掉，也就是$$pl(i-1,j)+ql(i,j+1)$$这样的，遇到一个手，它就直接往左飞。如果$$b=j$$需要特判。
