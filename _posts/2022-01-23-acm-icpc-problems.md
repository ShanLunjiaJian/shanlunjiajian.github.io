...
考虑怎么$$n^2$$，我们枚举一个区间$$[i,j]$$，然后看它和$$[j+1,j+1+(j-i)]$$是不是最多相差一个位置，如果是的话，$$[j+1,j+1+(j-i)]$$开始的所有方案都可以接到$$[i,j]$$后面。为了判断这个，可以sa求出它们的lcp和lcs，然后判断它们的和，或者直接维护有多少不同的。

考虑$$l$$很大的时候每一段都很长，所以容易想到根号分治或者调和数复杂度之类的东西。设$$p(i)=[s_i=s_{i+l}]$$，如果$$p$$中有两个很近的$$0$$，看起来就有很多区间没有了。如果两个$$0$$很远，这样的$$0$$个数很少。考虑了各种想法之后，发现了一个较为优美的做法，我们维护还没确定能否转移的部分的左端点$$p$$和$$p$$右边第一个$$0$$的位置$$i$$，那么$$i-p\leq l$$，一开始$$p=1$$。找到$$[p,p+L-1]$$中最右的$$0$$

 - 如果它就是$$i$$，也就是这里面只有一个$$0$$，那么我们就找到$$i$$右边第一个$$0$$，然后$$p$$会有一段可行一段不可行的，记录它并把$$p$$移动到$$i+1$$，此时$$p,i$$总增量$$\geq L$$。

 - 如果里面还有至少一个$$0$$，画一画可以发现，这种情况连续发生三次的话，$$p,i$$的增量都至少是$$L$$。

所以我们知道可以转移的位置是$$O(\frac{n}{L})$$个区间。
...
#### Stage 12: Grand Prix of Belarus
...
#### Stage 13: Grand Prix of Bytedance
...
考虑如何砍一个$$n$$。我们直接模拟一遍求出每种情况下每个位置的值，然后每个位置就是一个限制，那么我们从左往右扫序列，问题是有多少次操作覆盖了这个位置，然后就$$O(n)$$单次了。关于这个限制，具体一点，直接设三个序列的操作次数分别是$$l,m,r$$，当前扫到的位置分别被$$c_l,c_m,c_r$$次操作覆盖，那么每个限制是$$\min(l,c_l)+\min(m,c_m)\geq x,\min(m,c_m)+\min(r,c_r)\geq y$$之类的，每个限制可以拆成四个，最后变成$$l,m,r,l+m,m+r$$分别$$\geq$$某些数的限制，我们要最小化$$l+m+r$$，手动simplex枚举构成某个顶点的限制即可，具体一点就是
...
为了大概地证明这个结论，猜测对于任意策略，答案$$f(x)$$大概是超鞅(期望比当前值小)，因为如果它是严格亚鞅(是亚鞅而不是鞅)，我们不停赌当前值的一半似乎就赢了。超鞅也就是说$$f(x+k)p+f(x-k)q\leq f(x)$$，其中$$q=1-p$$。感觉上如果$$f$$能够全部取得等号，就无敌了，打表发现那个策略真的取到了，所以就赢麻了。
...
观察搜出来的这些置换，发现奇数总是在奇数位，偶数总是在偶数位，所以这样的置换只有$$5!^2=14400$$种，那么看起来就是说我们可以凑出其中一半。考虑能凑出来的和凑不出来的的区别在哪，根据某个agc题的经验，猜测和逆序对有关，我们搜一下$$n=6$$，发现逆序对数总是偶数。
...
考虑什么情况下第一天就有人跳。考虑第$$i$$个人跳了，当且仅当他知道存在一个不可能的序列，在除了他以外的位置和他看到的是一样的。

考虑第一天没有人跳的话，说明每个人眼中都没有一个不可能的序列满足剩下的人，那么也就是这些序列和剩下的人都有不同。此时如果第二天有人跳了，说明
