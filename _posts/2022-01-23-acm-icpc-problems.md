...
随。然后发现并随不出来。如果你运气足够好，你可以随出来的吧（

考虑继续钦点$$3(i+5)\leq a_i<3(i+6)$$，但是搜了一下$$n$$很小的情况发现无解，于是$$n=2000$$的时候必然没有这样的解。
...
考虑直接套用 模板 斐波那契循环节 的做法。求出循环节之后，我们只需要考虑一个循环内发生的事情，这个被缩减到了$$6p$$以内。

然后做法比较牛逼。考虑折半，我们知道$$f_{n+m}=f_{n-1}f_m+f_nf_{m+1}$$，类似于bsgs，我们取一个$$K=\Theta(\sqrt{p})$$，并枚举$$n<K,m=tK$$。

接下来，如果$$f_{n+m}\bmod{p}=x$$，那么$$f_{n-1}f_m+f_nf_{m+1}\bmod{p}=x$$
