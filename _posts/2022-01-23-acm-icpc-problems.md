...
#### Stage 14: Grand Prix of America
...
感受了一下，感觉第一步，也就是Three Indices和这个题共同的trick似乎是一个找到所有重串的算法。但是查了半天没有查到它。杜老师认为它是远古trick，所以就叫ancients' trick好了（

K. Three Dimensions

数位dp。虽然结束的很容易，但是结束的很困难。

#### Stage 16: Grand Prix of Moscow
...
经典地，考虑一开始先满上，然后每个位置可以把一个后缀减去一些，并且选的总量不能超过$$c_0$$，不妨钦点$$c_0=1$$。经典地，二分答案，然后转化成各$$d_p-kd_0\geq 0$$。那么此时相当于有若干个操作可以选，每个可以给各$$d_p-kd_0$$加上一个数，求是否有可行解。注意到这些操作张成的东西就是凸包，然后限制相当于拿一个面去切，必然可以切到一个点上，也就是说我们最多只会用一个操作。
