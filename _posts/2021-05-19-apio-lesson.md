...
我们考虑一个过程`solve(l,r,L,R)`，表示给出从$$(r,L),(r,L+1),...,(r,R)$$(右边竖线)出发的答案，计算从$$(l,L),(l+1,L),...,(r,L)$$(下边竖线)出发的答案。
...
。先递归右上，然后处理右下(处理1和2中右下对左下的贡献)，然后递归左下。右下的部分是一个长方形，贡献可以容易地用组合数表示，显然是一个卷积的形式，我们用法法塔优化，就做完了。

呃复杂度?每一层的$$R-L$$之和是$$O(n)$$的，一共有$$\log n$$层，每层的法法塔一共是$$O(n\log n)$$，一共就是$$O(n\log^2 n)$$。

啊那个用栈的结论，还有另一种推法。说的是考虑折线，把$$f_k(a)$$里面$$k$$个交点删去，剩下的就

-----

Chess Tournament

有一场大赛，有$$n$$位选手要两两进行一场比赛。大赛进行若干轮，每一轮最多有$$k$$场比赛，并且一名选手不能在一轮中参加多于一场比赛。设计一个方案使得比赛轮数最少。$$n\leq 200,k\leq\frac{n}{2}$$，2s 512MB。
...
按照上面的构造来放的时候，任何时刻被放的一定是一个前缀一个后缀和中间一些作为过中位数的数。同时，每个位置的上下界是越来越小的。

考虑从前往后扫，在一个数离开合法区间的时候确定它的值，这么做是因为合法区间中的数
...
所以，容易想到一个排列可以搞出来的一个必要条件 : 按前缀最大值分段，它的每一段长度都不超过$$3$$。

显然还有另一个条件 : $$1,2$$段数相同，且$$1,2,3$$段数和
...
考虑直接对于每种可能的大小关系预处理，不过$$10!$$太大了，会T飞。

如果可以把$$10!$$变成$$2^{10}$$就很好，同时注意到最后得到的数一定是操作数中的一个，所以我们枚举一个$$i$$，用上面的式子搞一搞，这是预处理。查询的时候直接做就好了。
