...
注意到莫队的重要性质是所有$$r^\prime-r$$的和是$$O(n\sqrt{q})$$的，所以我们可以对于每次求$$g$$爆力枚举$$i\in[l_2,r_2]$$。容易使用BiT维护，但是复杂度会多一个$$\log$$。

注意到只有$$n$$次插入，但是查询有$$O(n\sqrt{q})$$次，所以可以上一个根号平衡。注意这里不需要可持久化了，所以空间就是线性了，我们就解决了这个问题。
