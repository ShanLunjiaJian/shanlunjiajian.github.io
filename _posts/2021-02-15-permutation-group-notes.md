...
**引理4** 如果使用$$m$$种颜色对集合$$X$$的元素染色，那么对于置换$$f$$，在$$f$$作用下保持不变的染色个数为
...
面对一些更强力的问题的时候，上面的Pólya定理可能并不能搞定，或者会变得非常麻烦。于是我们引入下面更强的Pólya定理 : 

定义一个颜色$$\beta\in\mathcal{B}$$的权重$$w(\beta)$$是随便一个什么东西，可以是数，也可以是多项式或者矩阵之类奇怪的东西。别忘了$$\mathcal{B}$$是颜色集合。

定义一个染色$$\mathrm{c}\in\mathcal{C}$$的权重$$w(\mathrm{c})$$是它对被染色集合$$X$$中每个元素所染颜色权重的积。

定义一个染色等价类$$\phi$$的权重$$w(\phi)$$是它里面所有染色的权重。这些权重显然相等。
...
**更强的Pólya定理** 在置换群$$G$$的作用下，$$\mathcal{C}/G$$中所有染色等价类的权重之和是 : 

$$
\sum_{\phi\in\mathcal{C}/G}w(\phi)=Z(G;\sum_{\beta\in\mathcal{B}}w(\beta),...,\sum_{\beta\in\mathcal{B}}w^m(\beta))
$$

**证明** 略。My math is not tooooooooooooooooooooo strong.
