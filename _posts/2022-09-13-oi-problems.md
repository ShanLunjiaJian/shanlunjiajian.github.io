...
看起来很困难。考虑怎么求一个最小表示，考虑什么东西和这个结合的比较好，使用$$\log$$层duval求出所有significant suffix，一次可以几乎直接加入一段，然后比$$\log$$次就结束了。

或者直接考虑最小表示，每一段只有第一节和最后一节里面的后缀可能有用，求两个串的lcp可以二分之后讨论一下或者hash。
