...
注意到如果有至少$z$个人，我们必然尽可能早地放一个点，这样就是$O(n^2z)$了。于是希望把$i,j$砍到$O(\frac{n^2}{z})$的级别，但是不太行啊，毕竟摆渡车里面是$z=n$的，并没有砍到线性。

换个想法，首先这里$j>0$时的转移是确定的，所以我们直接把这一维扔掉，那么转移的时候就要枚举连续选多少次。
...
classic。考虑我们就是求一个字典序$k$小的方案，先把所有东西插入线性基