...
为了在环空间中求和它xor的第$k$小，如果生成树上的边是编号最小的那些我们就赢了，否则我们需要消出一个对角基来，但是这个巨大矩阵上大概不是很好操作。考虑能不能通过选择一棵特殊的生成树解决这个问题，我们选择字典序最小的生成树，那么对于每个基础环，上面最高的位一定是那条非树边贡献的。于是我们在此上贪心即可。$k$的改变只可能影响最低的$\log v$个基础环，爆力即可。求字典序最小的生成树使用kru。