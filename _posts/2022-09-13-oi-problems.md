...
考虑莫队，差分成四个 前缀和前缀的笛卡尔积，那么每次在正串dfn上移动，就是查询正串sam一个点和反串sam一个dfn前缀的笛卡尔积的权值和。

建立压缩sam。经典的(看起来至少在 子串统计 中出现过)，我们把每个点中包含的串，第一个beginpos作为横坐标，第一个endpos作为纵坐标画到平面上，这会得到一个上下轮廓线都单调的形状，并且压缩endpos的正串sam上每个点对应一行，压缩beginpos的反串sam上每个点对应一列。但是后面我就没看懂了。有机会再看吧!
