...
先给答案一个上界。考虑枚举终点，我们要求终点在一个环上，因为每个点都可以在$O(n)$步内走到一个环上。因为大家都会到终点，我们随便找一个终点所在的环，膜它的长度从终点出发跑同余最短路。这证明了答案不超过$n^2$。

于是考虑如果有点不在环上，枚举答案，我们要$01$矩乘$n^2$轮，每次乘法是$\frac{n^2}{w}$，要乘$n$次，然后全部and起来。复杂度$O(\frac{n^5}{w})$。

注意到答案的上界可以改成$n$乘上最小环的长度。考虑最小环的长度是不是跟边数有关，如果有关我们可能可以利用邻接矩阵稀疏的性质加速乘法的部分。但是很遗憾，我们可以构造一个一半是完全dag另一半是一条链的图，它的边数是$\Theta(n^2)$而最小环长度是$\Theta(n)$。

考虑上面那个图答案显然有$2n$的上界，考虑能不能给点更猛的剪枝。猜测如果最小环长度是$l$，那么最远的能到达的点必然能在$O(\frac{nl}{\sqrt{m}})$轮内到达。
