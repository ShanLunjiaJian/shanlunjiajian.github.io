...
猜测直接从左往右扫，贪心让新产生的逆序对数最小即可，证明考虑如果选更大的数肯定不优，因为是从前往后选；如果选更小的数，因为是$\geq$某个数的限制，把所有数改大不会增加逆序对数。
...
爆力怎么做?注意到匹配的部分，最长的的信息量足够给出所有短的，所以只需要记最长的即可。当最长的失配的时候，找到它这个前缀的一个 是别的路径的前缀 的后缀作为新的最长的。

那么一条路径会给路径上每个点贡献最多经过次数状态，这样状态数看起来是很对的。考虑如何优化失配的过程，二分hash，或者建个acam。此时一个度数很大的点被很多条路径经过还是有问题，考虑怎么优化建图一下，首先我们用主席树把acam完整地建出来(而不是只求出fail)，那么转移就是连到主席树的根，直接结束了。由于主席树上全是$0$权边，dij的时候可以特判掉，复杂度$O(m\log n)$。
