...
那么考虑做一些scheduling，插入一个数的时候会产生一对，我们找另一对，如果这两对有相同的那就让连通块个数$-1$了，否则我们知道不管什么数要来找一对不包含它的匹配，这两对里总有一对可行。也就是说插入一个数会让连通块数$-1$或者让自由的匹配对数$+1$，如果这个数找不到匹配则会让自由的匹配对数$-1$
...
然后可以猜这玩意是决策单调的，确实是这样，点越往左，最优决策点越往右。但是这里还有一个只能从某个区间转移的限制，不过关系不大
