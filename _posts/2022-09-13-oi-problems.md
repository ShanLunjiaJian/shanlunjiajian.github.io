...
那么接下来所有环的长度都是$\mathrm{ord}_{2l+1}(2)$的因数，我们爆力对每个环长$d$算一下贡献，于是每个查询都可以膜$d$，也就是说只有$d$种查询。对于每个查询，要查环上某些位置的和，以及每个位置带一个权的和，后者是循环卷积可以法法，前者扫描线扫查询位置，
