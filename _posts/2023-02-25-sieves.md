...
可以注意到$\mu^{\cdot 2}$的dgf是$\frac{\zeta(z)}{\zeta(2z)}$。考虑$\frac{1}{\zeta(2z)}$是个什么东西，它就是$\zeta(2z)=\sum\limits_i\frac{1}{(i^2)^z}$的逆，$\zeta(2z)$就是完全平方数的dgf，容易算出$\frac{1}{\zeta(2z)}=\prod\limits_p(1-\frac{1}{p^{2z}})$，也只在完全平方数处有值，或者你可以根据完全平方数乘法的封闭性来说明这件事。所以我们可以在完全平方数上整除分块一下做到$O(\sqrt{n})-O(n^\frac{1}{3})$。
