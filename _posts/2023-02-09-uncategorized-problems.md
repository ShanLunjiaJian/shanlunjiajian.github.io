...
注意到直径可以合并，考虑嗯分治，但是直接考虑直径比较麻烦，我们考虑外接邻域，两边分别是$\mathrm{N}(c_1,r_1),\mathrm{N}(c_2,r_2)$，$d=\mathrm{dis}(c_1,c_2)$，那么如果$d+r_2<r_1$或$d+r_1<r_2$则并的外接邻域是$\mathrm{N}(c_1,r_1),\mathrm{N}(c_2,r_2)$，否则直径必然是$r_1+r_2+d$，证明只需要考虑外接邻域的定义是，一个点集的外接邻域是$\mathrm{N}(c,r)$当且仅当有两个来自$c$不同子树的点到$c$的距离是$r$，且没有点超过$r$。