...
。注意到最里面是一个$$\exp$$求导的形式，也就是$$(\exp(F))^\prime=\exp(F)F^\prime$$，所以$$G_0^\prime-G_0F^\prime$$长度不超过$$2n$$，且前$$n$$项都是$$0$$，可以用长$$n$$的循环卷积计算，需要$$3\mathrm{E}(n)$$，而我们已经计算了$$\mathcal{F}_{n}(G_0)$$，所以只需要$$2\mathrm{E}(n)$$。

然后是$$H$$乘上那个东西，它的长度不超过$$3n$$，且前$$n$$项都是$$0$$，还是可以用长$$2n$$的循环卷积计算，需要$$6\mathrm{E}(n)$$，并且这一步计算了下一轮需要用到的$$\mathcal{F}_{2n}(H)$$。
