...
然后就不会了。然而做法很简单，考虑一个牛逼结论，我们选择的最大的数要么是$$a=\lfloor\sqrt[3]{m}\rfloor$$，要么是$$a-1$$。考虑如果选了$$t$$，那么$$x$$最多是$$(t+1)^3-1$$，于是最多只剩下$$(t+1)^3-t^3-1=3t^2+3t$$还可以用。如果我们选了$$a-2$$，那么剩下$$3(a-2)^2+3(a-2)$$，此时选的总和最大是$$(a-2)^3+3(a-2)^2+3(a-2)=a^3-3a^2-3a-2$$，选了$$a$$则至少是$$a^3$$(，选了$$a-1$$至多是$$a^3-1$$)。接下来只要证明选了$$<a-1$$的不能获得更大的块数，而这是显然的
