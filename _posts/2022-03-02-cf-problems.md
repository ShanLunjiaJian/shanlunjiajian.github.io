...
考虑爆力怎么做，我们枚举一个$$x$$，然后在膜$$p$$下计算这个方案数，然后你发现一个重排是合法的，当且仅当$$a_i\leq x+i$$，也就是$$a_i-i\leq x$$。

那么这个东西看起来好像是说，每个数$$a_i$$有一个可行的位置后缀$$[s_i,n]$$，其中$$s_i=\max(1,a_i-x)$$，然后这就是一个经典问题了，方案数就是把所有$$s_i$$从大到小排序之后$$\prod((n-s_i+1)-i+1)=\prod\min(n-i+1,n+x-a_i-i+2)$$的值。

然后考虑如何才能对于这些$$x$$快速计算这个。注意到$$p$$是素数，也就是说这个乘积为$$0$$当且仅当有一个$$0$$。同时注意到所有$$s_i$$顺序是不变的，所以先按$$a_i$$从大到小排序之后，我们得到若干个$$\min(n-i+1,n+x-a_i-i+2)\bmod{p}=0$$，这使得$$x\bmod{p}=a_i+i-n-2$$不可行。特别的，如果$$n-i+1\bmod{p}=0$$，此时$$n+x-a_i-i+2<n-i+1$$，也就是$$x<a_i-1$$。做完了。