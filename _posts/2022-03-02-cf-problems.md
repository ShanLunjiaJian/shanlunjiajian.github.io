...
考虑了一下，发现这俩都是拟阵，所以可以拟阵交/xia，但是我对拟阵的理论并不熟练，所以还是放弃这个好了。

肯定要把连通块缩起来。

考虑一个结论，我们最后必然可以把一张图变成连通的。考虑我们随便找到两个连通块，它们之间的边如果一条都不能加进去，说明这两个连通块在另一张图里包含于同一个连通块。于是如果不存在两个可以加边的连通块，说明另一张图就是连通的了。

然后刚才考虑拟阵交的时候，猜了一个结论是这个交还是拟阵，然后这么证好像并不能证明这个结论，所以我就输了。不过这个性质也比较接近拟阵了。但是我又感觉我没输!所以这个题会收录于 拟阵。

于是我们会做D1了，也就是一条边现在不能加那么以后都不能加，所以枚举每条边往里加就行了。
