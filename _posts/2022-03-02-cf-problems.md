...
每个格子都受到攻击让我们想到，这就是每个格子所在的行或列至少一个放了车。给每行每列建一个点，这就是说左右只有最多一边有孤立点。

然后你发现这个等价于要么每行有一个车，要么每列有一个车。这两个都满足的话，答案是$$n!$$，并且只会在$$k=0$$的时候出现，可以特判掉。然后我们钦点每行有一个车，每列的方案数是一样的。

接下来问题是，把$$n$$个车分进$$n$$列，使得每个非空列的车个数$$-1$$之和是$$k$$。考虑
