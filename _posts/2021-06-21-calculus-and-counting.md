...
考虑和。我们知道$f,g$分别满足$n,m$阶的ode，当然阶指的是导数的最高阶。考虑$(f+g)^{(k)}=f^{(k)}+g^{(k)}$，注意到$k\geq n$的时候可以利用ode消掉$f^{(k)}$，$k\geq m$同理，于是只有$n+m$个维度(每一维是一个有理分式)，于是$n+m$阶导内必然出现线性相关。

考虑积。我们知道$f,g$分别满足$n,m$阶的ode，那么类似上面消一消，每一项必然是$f^{(i)}g^{(j)}$的形式，于是只有$nm$个维度。

考虑复合有理分式。复合得到的形式必然是$(f(g))^{(k)}\sum\limits_{i<k}a_{k,i}f^{(i)}(g)$，其中$a$也是有理分式。于是只有$n$个维度。
...
看了一眼发现答案是$\frac{n!}{a!b!c!}[z^{\frac{n-a-b-c}{2}}]F(;a+1;z)F(;b+1;z)F(;c+1;z)$。从 https://loj.ac/s/951782 可以贺到EI的ode自动机，使用`ode_pFq()`构造三个超几何函数的ode，然后使用`DFinite_prod()`算即可，最后可以得到一个$8$阶ode。 https://atcoder.jp/contests/abc240/submissions/35762311 我的提交记录。
...
呃$$G$$是超几何函数，$$T$$是代数的，所以已经可以喂给ode自动机了。不过这里演示一下手动构造ode。当然先求个导。
...
由于这个式子据说太长了，dsq建议使用机器计算。我没有尝试过。
