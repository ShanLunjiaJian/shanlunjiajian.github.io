...
你发现上面的最后一段可以用来求微分有限生成函数$$F$$复合任意生成函数$$G$$。我们构造$$F$$的微分方程，然后就可以得到$$F(G)$$的微分方程，它必然也是线性齐次的，不过其中可能有卷积，可以用分治法法塔求解。

呃结合牛迭可以做到$$O(n\log^2 n)$$解任意微分有限生成函数的复合逆。这个咋搞?

你考虑直接牛迭一下。要求$$G$$的复合逆$$F$$，我们直接设就是设$$f(F)=G(F)-z$$，然后求导也很简单，主要问题就是要算微分有限生成函数复合任意生成函数，这就是上面的东西了。看起来常数不小。
