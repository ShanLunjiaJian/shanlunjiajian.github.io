...
呃实际上我们有手算的方法。考虑$$P$$的微分方程很简单，问题是怎么求$$P(Q)$$的微分方程。你发现直接代入$$Q$$可以得到一个微分方程，但是里面全是$$P^\prime(Q)$$或者$$P^{\prime\prime}(Q)$$这样的东西，而我们想要的是$$(P(Q))^\prime$$这样的。可以使用链式法则，它建立了复合的导数和导数的复合之间的关系，所以我们可以用$$(P(Q))^\prime$$表示$$P^\prime(Q)$$，然后代回去就得到关于$$P(Q)$$的微分方程了。然后法法塔就一个$$\log$$了。
...
呃结合牛迭可以做到$$O(n\log^2 n)$$解任意微分有限生成函数的复合逆，这个东西例题在下面，当然我还没找到。

-----

A111111 Simple Permutations
