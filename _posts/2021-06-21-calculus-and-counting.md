...
据说这个题的方法可以解一大车微分方程，但是我也不会，见着再说吧（
...
呃实际上我们有手算的方法。考虑$$P$$的微分方程很简单，问题是怎么求$$P(Q)$$的微分方程。你发现直接代入$$Q$$可以得到一个微分方程，但是里面全是$$P^\prime(Q)$$或者$$P^{\prime\prime}(Q)$$这样的东西，而我们想要的是$$(P(Q))^\prime$$这样的。可以使用链式法则，它建立了复合的导数和导数的复合之间的关系，所以我们可以用$$(P(Q))^\prime$$表示$$P^\prime(Q)$$，然后代回去就得到关于$$P(Q)$$的ODE了。然后法法塔就一个$$\log$$了。

-----

分治法法塔

你发现上面的最后一步可以用来求任意微分有限生成函数$$F$$复合任意生成函数$$G$$。我们构造$$F$$的微分方程，然后就可以得到$$F(G)$$的微分方程，它必然也是线性齐次的，不过其中可能有卷积，可以用分治法法塔求解。

呃结合牛迭可以做到$$O(n\log^2 n)$$解任意微分有限生成函数的复合逆。这个咋搞?

你考虑直接牛迭一下。我们直接设就是设$$f(F)=F(G)-z$$，然后求导也很简单，主要问题就是要算微分有限生成函数复合任意生成函数，这就是上面的东西了。看起来常数不小。

-----

Simple Permutations

定义一个排列是一个简单排列(simple permutation)，当且仅当它不存在非平凡的连续段，连续段就是值域连续区间，非平凡指的是长度不为$$0,1,n$$。求有多少长$$n$$的简单排列。$$n\leq 5\times 10^4$$，膜$$998244353$$，鬼知道几秒。
...
对应地，定义一个长$$m$$的排列$$\sigma$$的膨胀$$\sigma[\alpha_1,\alpha_2,...,\alpha_m]$$，其中$$\alpha_i$$也都是排列，为把$$\sigma$$中的元素从小到大替换成对应位置上的$$\alpha$$，当然后替换进来的需要整体加上偏移量。例如$$3142[12,1,1,2413]=67183524$$。

接下来做一些迷惑的事情。定义一个排列是正不可分的，当且仅当它不能由$$12$$膨胀得来，负不可分则是不能由$$21$$膨胀得来。定义$$i_n$$是长$$n$$的正不可分排列数量，负不可分排列显然也是这么多。

**结论** 对于任意长度不为$$1$$的排列$$\pi$$，都存在恰好一个长度不为$$1$$的简单排列$$\sigma$$，膨胀之后可以得到$$\pi$$。

如果$$\sigma$$的长度$$>2$$，那么$$\alpha_i$$们也是唯一的，$$=2$$时则有两种方案。如果$$\sigma=12$$，并且我们要求$$\alpha_1$$是正不可分的，那么$$\alpha_1,\alpha_2$$也是唯一的了；$$\sigma=21$$中则是负不可分。

**证明** 好猛啊。

先考虑$$\pi$$中存在两个不同的非平凡极长连续段，并且它们的交非空的情况。

你发现既然它们的交非空，那么它们的并一定也是值域连续的，所以它们的并一定是平凡的$$[1,n]$$，不然就不极大了。然后考虑其中一个连续段挖掉两个连续段的交，得到的应该还是连续段，于是我们知道$$\pi=\sigma[\alpha,\beta]$$，其中$$\sigma=12$$或者$$21$$。你发现这里有两种挖法，都可以得到一组$$\alpha,\beta$$，所以实际上我们有两种方案。

如果考虑一个排列中所有相交的非平凡极长连续段，你发现它们的结构是排成一排的，显然其中只有最左边那个挖掉它和次左那个的交，得到的$$\alpha$$是满足正/负不可分的。

然后考虑如果$$\pi$$中所有非平凡极长连续段都是不交的。这就像废话一样，我们取这些连续段就得到了唯一的分解方式。

然后你发现，这个结论建立了一个$$(\sigma,\alpha_1,...,\alpha_m)$$到任意排列的双射，其中$$\sigma$$是长为$$m\geq 2$$的简单排列，不过这里长为$$1,2$$的$$\sigma$$是特殊的，它们要求$$\alpha_1$$是正或者负不可分的，但是在计数中是等价的。
...
这个看起来很好。要解出$$S$$我们还需要消掉$$I$$，不过显然应该可以搞出这么一个方程，根据上面的双射可以得到这么一个东西
...
$$
S(z)=z-\frac{2z^2}{1+z}-F^{<-1>}(z)
$$

，其中指数上的$$<-1>$$表示复合逆。所以就是要算复合逆了。你发现$$F$$是超几何函数减$$1$$，所以微分有限，容易得到它的ODE，然后利用上面的科技就可以解决这个问题。

这个也是从论文里看的，[可以从这里找到](https://cs.uwaterloo.ca/journals/JIS/VOL6/Albert/albert.pdf)。其中有一些词我瞎翻译了，simple permutation 译为 简单排列，inflation 译为 膨胀，plus/minus indecomposable 译为 正/负不可分。有什么问题请联系我/jk