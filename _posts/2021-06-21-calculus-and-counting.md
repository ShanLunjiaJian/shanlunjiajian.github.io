...
在ODE自动机中我们需要维护这个$$G$$，它们的四则运算是可以搞定的，但是在我学ODE自动机之前我并没有兴趣学这一部分（

-----

线性齐次微分方程，微分有限和P-递归

如果一个幂级数满足一个线性齐次微分方程，那么它就是微分有限的，注意微分方程里的系数可以是各种东西。代数幂级数都是微分有限的。
...
，其中$$p_i$$是一个多项式，这个形式被称为P-递归。事实上，一个幂级数的系数是P-递归的，当且仅当它微分有限。

如果你并不理解怎么通过ODE推出来递推式，你可以看一看第一个题，也就是 排列。
...
展开之后我们得到一个$$\max(n,m+1)$$阶的线性齐次微分方程，而它当然可以直接给出一个递推式。
...
这一方法的优秀之处在于，它可以处理一大类常见的生成函数，并且大大简化了人力计算的过程。当然也有很多生成函数它是搞不定的。
...
另外这个题的$$n$$可以做到非常大，比如1e18，因为我们有快速的整式递推。
...
呃看起来如果直接提取系数的话，这个ODE好像确定了一个递推关系。
...
呃这个怎么跟$$G$$联系起来呢......试一试硬凑的话，~~不~~可以发现有这么一个奇怪的式子 : 
...
呃现在瓶颈在......快速幂。不过我们可以通过线性逆元和线筛幂做到接近线性。
