...
呃$$G$$是超几何函数，$$T$$是代数的，所以我们可以构造ODE得到线性的递推式。先导一个试试。
...
看起来现在可以解ODE来得到$$G$$了，不过没有必要，因为我们只想通过ODE来递推/cy
...
另外这个题的$$n$$可以做到非常大，比如1e9，因为我们有快速的整式递推。
...
你发现上面的最后一段可以用来求微分有限幂级数$$F$$复合任意幂级数$$G$$。我们构造$$F$$的微分方程，然后就可以得到$$F(G)$$的微分方程，它必然也是线性齐次的，不过其中可能有卷积，可以用分治法法塔求解。

呃结合牛迭可以做到$$O(n\log^2 n)$$解任意微分有限幂级数的复合逆。这个咋搞?

你考虑直接牛迭一下。要求$$G$$的复合逆$$F$$，我们直接设就是设$$f(F)=G(F)-z$$，然后求导也很简单，主要问题就是要算微分有限幂级数复合任意幂级数，这就是上面的东西了。看起来常数不小。

-----

Simple Permutations

MiFaFaOvO的blog里说是NEERC 18 I，但是我也没找到（
...
你发现，如果我们想把一个排列分成尽量少的连续段(当然不要长为$$n$$的)，从全是长为$$1$$的连续段开始合并，最后一定会合不动了，此时把所有连续段拿出来排序(排列中两个连续段一定可比)，得到的排名数组应该是一个简单排列，换句话说这些连续段的大小关系等于一个简单排列中的大小关系。啊怎么证明?你考虑如果不简单，那么说明还可以继续合并连续段。
...
要解出$$S$$我们还需要消掉$$I$$，不过显然应该可以搞出这么一个关于$$I$$的方程。你考虑要生成全部排列和要生成正不可分排列的区别，就在于$$\sigma$$不能取$$12$$了，所以就是上面式子中间那个系数$$2$$变成$$1$$了，我们得到
...
。接下来是最关键的一步 : 换元，把$$z$$换成$$F^{\langle -1\rangle}$$(就是$$F$$的复合逆)，这样就把$$S(F(z))$$变成$$S(z)$$了
