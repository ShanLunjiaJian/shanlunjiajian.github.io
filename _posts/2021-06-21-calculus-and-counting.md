...
称一个幂级数$$F(x)$$在$$K[x]$$上是代数的，当且仅当在$$K[x,y]$$上存在二元非零多项式$$G(x,y)$$，使得$$G(x,F(x))=0$$，也就是$$(x,F(x))$$是$$G$$的一个根。在ODE自动机中我们需要维护这个$$G$$，它们的四则运算是可以搞定的，但是在我学ODE自动机之前我并没有兴趣学这一部分（

-----

常系数线性微分方程，微分有限和递推

如果一个幂级数满足一个常系数线性微分方程，那么它就是微分有限的，注意这里的常系数可以是常多项式。根据这个ODE容易推出它系数的递推式。如果你并不理解，你可以看一看第一个题，也就是 排列。
