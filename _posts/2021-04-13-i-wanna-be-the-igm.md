...
枚举一个$$i$$，然后从高到低按位贪心，维护一个当前最优的$$s=a_j\ \mathrm{and}\ a_k$$。这个$$s$$的含义是，存在一个$$a_j\ \mathrm{and}\ a_k$$包含$$s$$。

如果$$a_i$$这一位是$$1$$，那么$$s$$这一位是$$0,1$$都随意。根据$$s$$的含义直接赋$$0$$即可。

如果$$a_i$$这一位是$$0$$，那么我们希望$$s$$这一位是$$1$$。

所以我们的问题变成快速检查一个$$s$$是否可行，也就是是否存在一个$$a_j\ \mathrm{and}\ a_k$$包含$$s$$。
...
然后我就想起来一道题叫 SP2878/UVA1364 Knights of the Round Table。说的是一个边双里面如果有奇环，那么每个点都在一个奇环中。于是我就在奇怪的思路上越走越远!然后就去看题解了。

先随便跑一棵dfs树，然后对于每条非树边，看它有没有跟树边形成一个奇环，也就是它端点的深度差是不是偶数。
