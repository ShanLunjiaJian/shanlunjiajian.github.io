...
考虑配凑，我们枚举一个$$k$$，算有多少$$\geq k$$的。于是可以设$$dp(i,j,lim)$$表示前$$i$$位，有$$j$$位$$\geq k$$，懂的都懂的方案数。这样就是$$O(10^2n^2)$$。
...
考虑整数就是多重集，and卷积的时候我们用法嘛塔(子集后缀和)，这里我们也可以用法嘛塔(狄利克雷后缀和)。我感觉广义的莫比乌斯变换应该就是，偏序集上的前/后缀和及其逆吧，所以这么说好像完全没问题。
...
有一个[来自cf用户 TheScrasse 的做法](https://codeforces.com/blog/entry/89319?#comment-778197)。妈呀他的头像好棒啊（
...
这个翻不翻看起来就很像图论问题。考虑我们往相交而不包含的线段对连边，这样就得到一张图，如果只有两种翻法，就说明它是二分图。然后我们对每个连通块的左右部分别求翻的代价和，取小的即可。

现在问题是怎么才能快速找到这个二分图的每一个连通块，以及它们的左部和右部。毕竟我们边多的很。

考虑一个满的区间$$[1,n]$$，和它里面的$$\frac{n}{2}$$条线段。

 - 如果存在线段$$[1,n]$$，它不和任何线段连边，递归即可。

 - 否则，一定存在线段$$[1,x],[y,n]$$，其中$$y\leq\frac{n}{2}<x$$。其它每条线段必须包含于这俩中的一个，否则就会形成三元环，导致它不是二分图而无解。

   同时我们上面说过每条线段一定跨过中点，所以其它线段都与$$[y,x]$$相交或被它包含。

    - 对于包含于$$[1,x]$$并且与$$[y,x]$$相交的，它们都和$$[y,n]$$有边，因此它们之间没有边，否则就会形成三元环。这说明它们一定是大的套小的。

    - 对于包含于$$[y,n]$$并且与$$[y,x]$$相交的，它们都和$$[1,x]$$有边，因此它们之间没有边，否则就会形成三元环。这说明它们也一定是大的套小的。

    - 对于包含于$$[y,x]$$的，它们不和$$[1,x],[y,n]$$连边。同时如果它们和上两类线段有边，递归即可。

你发现我们的递归是只递归一次的，所以可以直接维护递归栈的信息，根据这些容易快速处理所有线段，使用计排的话复杂度$$O(n)$$。