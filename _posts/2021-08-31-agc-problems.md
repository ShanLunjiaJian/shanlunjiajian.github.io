...
然后考虑如何转移。填空的话考虑了很久，并不是很有前途，于是我们考虑插入。枚举填了$$k$$个有效1，那么它可以从$$dp(i-1,...)$$转移过来，用组合数选一选就行了。统计答案的话，就枚举哪些行没有1，组合数选一选。

怎么选?

 - 如果$$k=0$$，那么啥也没做。

 - 如果$$k=1$$，那么我们随便选一个位置就行了，不管怎么选它总是这一列的第一个和最后一个。

 - 如果$$k\geq 2$$，考虑我们枚举两边放在哪，中间隔板。于是可以写出式子$$\sum\limits_{l=0}\sum\limits_{r=}$$