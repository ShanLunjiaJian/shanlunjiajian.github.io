...
AGC021F : 还是数数题。看起来只有数数题能达到agc EF难度（

这个行列太奇怪了。我们重新描述题意。

$$n$$行$$m$$列的01矩阵。定义两个矩阵本质相同，当且仅当每一行第一个1的位置相同，每一行最后一个1的位置相同，每一列第一个1的位置相同。求有多少本质不同的矩阵，$$n\leq 200,m\leq 8000$$。

复杂度看起来是$$nm\log$$或者$$nm^2$$之类的东西。

考虑怎么直接dp。

如果一行一行决策，我们设$$dp(i,j)$$表示$$i$$行，有$$j$$列已经有了一个1的方案数，然后你发现我们并不能处理这一行的第一个1，和每一列的最后一个1。

如果一列一列决策，我们设$$dp(i,j)$$表示$$i$$列，有$$$$