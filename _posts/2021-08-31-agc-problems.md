...
考虑如果存在两个点$$u,v$$满足$$\mathrm{N}(u,d_1)=\mathrm{N}(v,d_2)$$，那么我们把$$u$$向$$v$$移动一条边得到$$w$$，$$\mathrm{N}(w,d_1-1)$$必然也和它们相等，除非$$u,v$$直接相邻了。于是每一个邻域$$S$$都存在一个最小的半径，并且恰好一个半径最小的中心。我们就在这个中心处统计这个邻域。

那么$$\mathrm{N}(u,d)$$计入答案的充要条件就是，不存在一个$$v$$使得$$\mathrm{N}(u,d)=\mathrm{N}(v,d-1)$$。注意如果考虑上整棵树会出问题，所以$$\mathrm{N}(u,d)$$不能是整棵树。
