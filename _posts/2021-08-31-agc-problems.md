...
找不出阶段，我们考虑是不是扫描线方向出了问题。发现我们可以每次找到SG值最小的点来转移，也就是把所有连到SG值最小的点的点的SG值更新一下，可以设$$dp(S,i)$$表示$$S$$中所有点SG值都更新到$$i$$了的方案数，那么我们可以枚举图中SG值是$$i-1$$的点$$T$$，那么$$S$$中的每个点都需要可以连到$$T$$中的一个点，这个可以简单预处理。最后只剩下一个点的时候，$$i$$必然是这一状态统计到的所有方案中这个点的SG值。做完了，复杂度是$$O(n3^n)$$，瓶颈在于预处理转移所需的那个东西。