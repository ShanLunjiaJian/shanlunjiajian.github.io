...
二分一波，然后考虑一个爆力dp，设$$dp(u,i,j)$$表示冲进$$u$$子树并走完所有叶子，第一个走深度为$$i$$的，最后一个走深度为$$j$$的，是否有可能。这是完全二叉树，所以遍历儿子的顺序只有两种，都试一下就好了。

如何优化?首先$$j$$越小越好，所以我们直接设$$dp(u,i)$$表示最小的$$j$$，不可行则为$$\infty$$。

然后可以看出一个单调性，也就是如果一个状态$$i$$和$$dp$$值都比另一个大，那么它没有用，这样按照$$i$$从小到大来看，所有的$$dp$$值就都是