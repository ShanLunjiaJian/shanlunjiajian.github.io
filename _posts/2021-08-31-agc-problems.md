...
首先考虑在中间统计答案，也就是算每个点走到两边矩形的方案数，然后乘起来求和。然后你发现这个好像很难做（
...
草，为什么这个东西这么简单/jk，那么一个点到一个矩形$$[l_1,r_1]\times[l_2,r_2]$$就是$$\binom{r_1+r_2+1}{r_1+1}-\binom{r_1+l_2+1}{r_1+1}-\binom{l_1+r_2+1}{l_1}+\binom{l_1+l_2}{l_1}$$了。

那么一个矩形到一个矩形呢?呃看起来比较恶心心。我们
