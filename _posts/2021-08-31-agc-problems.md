...
这个看起来好复杂啊，考虑一个递推，我们设$$dp(i)$$表示$$i$$最小出现在哪，那么实际上$$dp(i)=1+\max\limits_{j\rightarrow i}$$，可以用线段树维护。不过这是错的，因为我们需要把别的点也插进来![/jy](/img/qqemoji/0.gif)
...
AGC005E : 感觉思维难度远高于F![/jy](/img/qqemoji/0.gif)
...
AGC010E : grass，我以为是只换一次![/jy](/img/qqemoji/0.gif)
...
![img](/img/2021-08-31-agc-problems/agc015e.png)
...
转移枚举$$S$$中SG值最小的点$$T$$，注意$$T$$要么同时包含$$1,2$$要么同时不包含。然后我们钦定$$T$$内部的边都不能连，$$S-T$$到$$T$$的边先给$$S-T$$中每个点钦定一条然后剩下随意，设$$c(S,T)$$表示
$$S$$到$$T$$的边数，那么连边的方案数是$$\prod\limits_{i\in S-T}(2^{c(i,T)}-1)$$。
...
AGC017C : 注意到这是AGC，所以我们猜测这题没有数据结构![/jy](/img/qqemoji/0.gif)
...
$$
\begin{aligned}
&=\sum_{i=0}^{l_1}\binom{i+l_2+1}{i+1}
&=\binom{l_1+l_2+2}{l_1+1}
\end{aligned}
$$
...
考虑如果翻$$[l_1,r_1],[l_2,r_2]$$得到的串相同，那么
 ...
 - 如果它们包含，那么讨论一下，它们不一定都是回文子串![/jy](/img/qqemoji/0.gif)，比如`aabca`中翻$$[1,5]$$和$$[2,4]$$是一样的
...
AGC020E : 100是什么鬼数据范围![/jy](/img/qqemoji/0.gif)
...
$$
\begin{aligned}
dp(l,r)=\sum_{l\leq i<r}dp(l,i)g(i+1,r)\\
g(l,r)=\sum_{d\text{是循环节}}dp(l,l+d-1)
\end{aligned}
$$
...
$$\gcd(a_i,S-a_i)\neq 1$$是个啥?注意到$$\gcd(a_i\perp s-a_i)=\gcd(a_i\perp s)$$。这就非常好?
...
AGC022C : $$n,v\leq 50$$![/jy](/img/qqemoji/0.gif)
...
注意到总是需要等火车，区别在于是等走过去的还是走回来的。我们可以容易地对每个商场求出，坐正向/反向的火车到这个商场，需要花费多长时间才能买完东西并等到火车过来，火车过来的时候是往哪个方向开，于是每个商场的效果就是消耗一些时间，并且可能会改变火车的运行方向。

考虑有没有什么结论。大胆猜测，我们每次上车，遇到第一个没买的商场就会去买。

然后你发现这显然是假的，不然这题就变成模拟了。但是为什么它是假的呢?

尝试证明它是真的。如果我们没有买遇到的第一个，而是买了后面的某一个，那么我们总要回来买这一个的，而如果跳过它更优，只可能是一会再回来买可以改变方向。

考虑我们最后要回到$$1$$，所以车一定是跑了整数轮，所以一个商场的效果不需要是消耗了一些时间，而可以是增加了一些轮数。

正反到达一个商场，消耗的轮数相差不会超过$$1$$，于是如果一个商场正着比反着更快，我们希望正着到达，否则希望反着到达。然后好像看到了最短路，但是实际上这是最短哈密顿回路（
