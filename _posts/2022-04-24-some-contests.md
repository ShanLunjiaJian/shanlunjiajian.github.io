...
考虑能不能搞一个贪心。强行考虑模拟费用流，发现这个限制相当于$$[l,r]$$中选一个，$$[l+2,r]$$中再选一个，$$[l+4,r]$$中再选一个......所以我们就得到一个二分图最大权最大匹配。
...
考虑sam，发现一个区间的前缀是自动机上的一条链，后缀是parent tree上的一条链，所以问题就是求它们的交。考虑dag剖分，然后就结束了。
...
所以我们知道必然只有$$13$$左右个人进去了，那么每次就必须比出一位的大小关系。考虑如果上一个人看了A的某一位，我进去看了B的这一位，我就比出了这一位，而下一个人进来之后，此时我给他留下B的下一位，他只要看A的下一位就好了。
...
那么可以考虑区间最后被分成若干段，两边需要特殊处理，而考虑中间，两个选了的数中间必然是一个
...
考虑dp，发现转移就是把儿子们卷起来，然后$$c$$次项乘一个$$c$$拍起来。发现这个乘一个$$c$$拍起来就是求导之后代入$$1$$求值，也就是说什么$$\displaystyle dp(u,1)=\left(\prod_v(dp(v,0)+dp(v,1)z)\right)^\prime(1)$$

一个牛逼转化是，改成选择一个儿子传上去，那么显然答案是不变的。
