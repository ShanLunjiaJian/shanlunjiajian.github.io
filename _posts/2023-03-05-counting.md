...
定义$n_q=\frac{1-q^n}{1-q}$，$n!\_q=\prod\limits_{i=1}^ni\_q$，类似可以定义下降幂${n^\underline{k}}_q$。于是可以定义$q$-二项式系数$\binom{n}{k}\_q=\frac{ {n^\underline{k}}_q}{k!_q}$。称为$q$-analog，是因为我们取$q\rightarrow 1$时就能得到一个不带$q$的经典等式。
...
它本身是$(1-z)^n$的$q$-analog，并且可以看出它跟阶乘幂也有些关系，有$\frac{(q^n;q)_k}{(1-q)^k}={n^\underline{k}}_q$。这个形式在整数分拆之类的问题中常常出现。下面是$q$-二项式定理
...
显然$[z^0] (z;q)\_n=1$，那么$[z^k] (z;q)\_n=\prod\limits\_{i=1}^k q^{i-1}\frac{1-q^{n-i+1}}{q^i-1}=(-1)^iq^\binom{k}{2}\frac{ {n^\underline{k}}_q}{k!\_q}$。
...
另一个不那么常用的式子。考虑$n$为负数的情况，我们用$(z;q)\_n=(1-q^{n-1}z)(z;q)\_{n-1}$来定义它，那么可以知道

$$
(z;q)_{-n}=\prod_{i=-n}^{-1}\frac{1}{1-q^iz}=\frac{1}{q^\binom{n+1}{2}}\prod_{i=1}^n\frac{1}{q_i-z}
$$
