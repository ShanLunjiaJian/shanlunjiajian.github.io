...
考虑凑出右边的形式，那么这里$\frac{z}{G}=1+z+z^2$，也就是说$G=\frac{z}{1+z+z^2}$，那么$G^\prime=\frac{1-z^2}{(1+z+z^2)^2}$，$\frac{zG^\prime}{G}=\frac{1-z^2}{(1+z+z^2)}$，所以$H=\frac{1+z+z^2}{1-z^2}$。考虑$G$的复合逆$F$，有$\frac{F}{1+F+F^2}=z,zF^2+(z-1)F+z=0$，解得$F=\frac{1-z\pm\sqrt{-3z^2-2z+1}}{2z}$，显然根号里面的常数项是$1$，分子的常数项如果消不掉这就不是多项式了，所以应该取减号对吧。那么答案是$H(F)$，所以又所有东西都是代数的了。根据$F$的定义它就是$\frac{F}{z(1-F^2)}=\frac{F}{(z-1)F+2z}$。
