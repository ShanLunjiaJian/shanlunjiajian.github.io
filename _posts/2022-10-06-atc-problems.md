...
考虑强行做，我们先考虑$a_{i_1}$，钦点它是$1$，然后考虑$a_{i_2}$，它有可能是$1$也有可能是$2$。比如$i$是$1,2,1,1$，那么$a_2$必须是$2$。注意到一个数连续多次出现好像很有用，如果没有一个数连续出现两次，那么所有数出现的时候就可以$=$当时的$\min$，那么考虑如果一个数某刻连续出现两次了$\min$，就把和它相同的数都$+1$，每个数加两次就无解了所以复杂度是线性。最后check一遍。