...
这个东西复杂度怎么看都非常见鬼，分析起来显然也会很困难(它和不可持久化的版本有区别)，但是书中猜测它是对的，并且他们好像做了实际测试，在各种大数据下速度接近不可持久化的配对堆。
...
求值规划实在是非常简单。对于可持久化二项堆，每次操作之后势能减少$$k-1$$，但是我们创建了$$k+1$$个懒惰求值，于是我们开一个队列，处理掉前面产生的两个懒惰求值即可。
