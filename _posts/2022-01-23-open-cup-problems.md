...
贪心地，每次征服能征服的里面最大的，然后立即动员它，直到你的兵力足够征服所有星球。
...
就是选一个子集满足最大值$<$剩下的的和，那么每次删去最大值直到满足这个条件即可。
...
先求出每条边何时断掉，然后二分答案嗯check就好啦。那么怎么求？？？对度数根号分治，改小点爆力，改大点爆力周围的大点，周围的小点怎么办？？改小点的时候在周围的大点处挂一个标记，标记是个区间，表示如果不在这个范围内就断了，复杂度$O(n\sqrt{n}\log n)$。你的时限比较松弛，但你的$\log$弥补了这一点。

哦哦这是个平面图啊，平面图满足$m=O(n)$，我们每次找到度数最小的点，让改它的时候爆力找周围的点，并在周围的点处挂个标记，然后把它删掉，这样就是$O(n\log n)$了。

F. Fantastic compression

还挺有意思的，$k$很小所以考虑$k$相关的指数级算法，注意到通过消元我们可以用第$i$个表示第$i+k$个，所以可以用前$k$个表示剩下的所有，然后$\bmod k$的每个等价类有一个最小的可能当$1$，枚举是哪个，然后我们找到剩下的最小的那个数，再枚举哪个等价类里最小的是这个数，复杂度$O(nk!)$。

G. Bookstore

也就是随机数据，每次给一个值域区间，求有多少序列区间的所有值包含其中。如果不随机，莫队就好了，并且看起来是强于小z的袜子的，所以也只能莫队了。但是现在随机，那么我们知道前缀$\min,\max$期望都只有$\log$个，所以枚举左端点这么算，算出来只有$O(n\log n)$种值域区间。

H. Cheese Game

可以区间dp。复杂度$O(n^3)$。

注意到每个人不可能操作超过三轮，如果超过三轮的话，第四轮就必然有一个数和第一二三轮选的数都有相邻，否则完全可以在一二三轮中就把它选走。但是一个数只和两个数相邻，所以这是不可能的。

注意到如果一个人的两轮分别选了一个数左右的两个数，那么另一个人下一轮可以直接把中间这个数选走，所以其实Bob不可能操作超过两轮。

注意到如果Alice第一轮留下了一个长$\geq 4$的段，那么Bob至少可以选走其中$1,3,5,...$大的，那么Alice第一轮就选走第$3$大必然不劣。如果留下了长$3$的段，Bob肯定会选$1,3$让Alice选$2$，所以不如直接选$2$。所以只会剩下长$1,2$的段。长$2$的段Bob会获得较大的，长$1$的Bob则会获得整个，看起来还是长$2$的好。最后可能要讨论一下。

I. Henry Porter and the Palindromic Radius

马库斯·拉迪乌斯·帕林多姆斯·布莱克叔叔。
...
肯定没法枚举一对天线，但是我们可以枚举一个，然后再枚举一个查询对吧，这样就变成一个角的数点了。

这个角有没有啥特殊的地方啊？？？它的两条边都是这个多边形的一个点和这个天线的连线。所以枚举多边形的两个点，分别极角排序，就能交出，或者说bound住我们所要的东西了。复杂度$O(n^2m\log m+q)$。

K. Ghost

什么b计算几何。把时间作为$z$轴考虑，那么每幅画的轨迹是一个凸的多面体，我们只需要做半空间交，就可以求出哪些时刻的哪些点是被所有画覆盖的，而半空间交也是凸的，这也说明这样的时刻构成一个区间。

考虑交出每幅画的轨迹的那些半空间，每个面要么与$x$轴平行，要么与$y$轴平行，所以我们对这两类分别做半平面交，然后扫描线扫$z$轴，维护当前答案多面体的截面即可，它一定是个长方形。

然后怎么求区间最大值，因为答案是上凸的，我们都不用线段树，只需要看看是不是包含全局最大值就好了，如果不是的话就在端点处取得。

L. Donuts

算一下全存short也要95m，也就是说我们必须$o(n)$空间。

是真的吗？？？值域很小，可以直接bitset存哪些位置的点存在了，这个只需要3m。

甜甜圈就是圆环嘛。

因为$l,r$是整数，我们求出$x$最大和最小的点，那么它们的中点就是圆心了，此时也知道了$r$。然后看点数就可以知道$l$，就是说随着加点，$r$是单增的，我们每到一个新的$r$就求出此时$l$应该是多少，然后就可以双指针维护$l$应该是多少，这部分总代价是$O(v^2)$。如果点数对了，还要继续检查是不是就是那个圆环。

考虑给点集做集合hash，但是我们不好快速算出圆环的hash值。

考虑看成01矩阵做字符串hash，此时只需要算出任意一个点为圆心的圆环的hash值，就可以简单乘除$O(1)$平移到另外的任意一个点了。这就行了，复杂度$O(n)$。
