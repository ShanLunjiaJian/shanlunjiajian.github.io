...
这个题还是比较带劲!首先注意到如果$k=i$的时候某个数留了下来，那么$k>i$的时候必然也会留下来，并且每个人操作之后都是这样的，所以我们每次找到多出来的留下来的那个数就好了。更带劲的是，一个人放进来一个数$x$并取走$\max$，相当于一开始取走$x$，然后依次考虑每个数$y$，如果$y>x$就交换$x,y$。这是扫$b$得到的过程，扫$a$也是可以的，而由于之前留下来的数之后还会留下来，每次只需要插入新的那个$a_i$。
