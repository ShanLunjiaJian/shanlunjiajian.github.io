...
做了一个上古hdu多校博弈论题(09多校day3 E)，然后发现自己爆了（
...
首先我们有一个最简单的超现实数$$\{\vert\}$$，它称为$$0$$，显然是一个先手必败的局面。
...
简单性法则指出，如果存在$$z$$使得$$x_L<z<x_R$$，那么$$x$$的值就是所有$$z$$中最简单的那一个。

那么什么是最简单的?超现实数的bst上只有所有形如$$\frac{a}{2^b}$$的数，我们定义最简单的是$$b$$最小的，也就是深度最小的。这样我们可以在bst上尝试逼近$$x_L,x_R$$来计算一些超现实数对应的数值，看起来复杂度是$$\log v$$。

注意这个主要适用于超现实数，因为只有超现实数才能用一个数代替，一般的博弈是难以用数代替的，从而难以直接比较孰优孰劣。
...
sg定理说明了每个游戏都是一个值，多个游戏一起进行，得到的值是它们值的xor。
