...
2-PN比起一般的PN多了哪些数呢?实际上如果一个PN $$x$$不包含质因子$$2$$，那么$$2x$$是2-PN而不是PN，除此之外2-PN不比PN多任何数了。所以2-PN的数量不超过PN数量的两倍，也就是说$$n$$以内2-PN数量也是$$O(\sqrt{n})$$。同样我们可以定义$$S$$-PN为去掉质数集合$$S$$中次数限制的PN，容易发现如果$$\vert S\vert=O(1)$$，那么$$S$$-PN的数量也是$$O(\sqrt{n})$$的。
