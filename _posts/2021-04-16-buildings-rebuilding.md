...
所以说，类楼房重建线段树就是通过力度巨大的剪枝，实现线段树上$$O(\log n)$$查询我们需要的信息。这个信息本来可能是需要$$O(len)$$预处理的，此时相当于以一个$$\log$$的代价支持修改(或者砍空间)。

当然这要求信息性质比较优秀。具体怎么优秀?不好说......

总之就是剪枝条件足够强，可以把复杂度剪成正确的，每次只递归一个儿子。

应用这一算法的常见标志有，前后缀min/max，单调栈(实际上是一个东西)，每个点$$O(len)$$的信息，等等。
...
例题1.7 CF671E Organizing a Race

CF上的3200+ Div.1 EF之类的题，很多有着两三四步的转化，需要非常强大的直觉和清晰的推理才能切出来。反正我不会，直接抄官方题解。

爆力怎么做?容易想到一个简单贪心，我们从左往右贪心地操作，也就是到走不动了再操作；从右往左则一口气加到起点。直接实现是$$O(n^3)$$的。

考虑 走不动了 这个东西好像比较好玩。具体地我们设$$next_i$$表示从$$i$$出发一直走到哪里才走不动了，当然如果可以一直走到最后那就$$next_i=n+1$$。

$$next$$怎么求?考虑设$$pre_i=pre_{i-1}+g_i-w_i$$表示一个前缀和，那么$$i$$右边第一个$$pre_j-pre_{i-1}<0$$的$$j$$就是$$next_i$$。

同时我们可以求出$$need_i$$，表示从$$i$$出发要跨过$$next_i$$至少需要多少次操作。显然有$$need_i=pre_{next_i}-pre_{i-1}$$.

你发现$$next$$形成了一棵树，让我们考虑一下它有什么性质。显然有每个位置往右走走走，在前面的贪心算法中，需要在树上每个父亲处付出$$need$$的代价，这相当于一个边权。

为了处理往左走，我们继续设$$suf_i=suf_{i-1}+g_i-w_{i-1}$$，注意这里还是前缀和，只不过换了个方向。

注意到$$j$$可以向左走到$$i$$，当且仅当对于$$i\leq k\leq j$$，都有$$suf_j-suf_{k-1}>0$$。同时，做一次操作，会给整个$$suf$$做一个后缀$$+1$$。

所以说，这题的关键在于转化成数据结构问题，而线段树的部分并不复杂。

lk给出了一个$$O(n\log n)$$做法，顺便说一下。