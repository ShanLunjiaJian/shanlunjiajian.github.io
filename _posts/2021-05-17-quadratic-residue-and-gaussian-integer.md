...
Cipolla先找到一个$$w$$使得$$w^2-n$$是非二次剩余。然后扩域，令$$\omega^2=w^2-n$$，则$$(w+\omega)^{\frac{p+1}{2}}$$就是$$n$$的一个离散平方根。

要找到这个$$w$$，容易发现因为二次剩余有恰好一半，期望随机两次就可以得到一个$$w$$。
...
**定理** Cipolla是正确的，或者说$$(w+\omega)^{\frac{p+1}{2}}$$确实是$$n$$的离散平方根。

**证明** $$(w+\omega)^{p+1}=(w+\omega)(w+\omega)^p=(w+\omega)(w-\omega)=w^2-\omega^2=w^2-(w^2-n)=n$$。

![/jy](/img/qqemoji/0.gif)所以直接搞就好了。复杂度是$$O(\log p)$$。
...
呃你说怎么搞膜$$2^k$$的?你发现$$n^{2^{k-1}}$$就是一个解，我不知道怎么来的，不过反正就是抄一下，对不对都没关系。
...
这里先只给出定义，高斯素数的判定定理是基于费马二平方和定理的，这个定理的证明则要用到高斯素数的概念![/jy](/img/qqemoji/0.gif)
