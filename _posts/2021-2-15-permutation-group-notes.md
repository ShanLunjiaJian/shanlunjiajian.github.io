...
至于某一类型的点置换个数，考虑这相当于给每个点染成$$1,...,n$$的一种颜色表示它所在循环的大小，然后要求颜色$$i$$用了$$ie_i$$次；接下来每个大小为$$i$$的循环还有$$(i-1)!$$种方式进行一个圆排列；接下来如果两个循环大小相同，它们是不做区分的，所以还要除一个$$\prod e_i!$$，一共就是

$$
\frac{n!}{\prod s_i\prod e_i!}
$$
