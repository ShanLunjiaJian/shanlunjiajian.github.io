...
首先由于对称性，每个循环的大小肯定是相等的，也就是说我们只要求出一个循环的大小，就可以求出循环个数。我们考虑设$$x$$为一个循环的大小，$$y$$为沿着一个循环走，走回起点的时候走了多少圈，那么有$$ix=ny$$，并且$$x$$是这个方程的最小正整数解。你随便证一证就知道最小正整数解肯定是$$x=n/\gcd(n,i), y=i/\gcd(n,i)$$，所以就做完了。至于怎么随便证一证，质因数分解然后反证法即可吧（
