...
那么如果你不知道，置换群是什么呢?群大家都知道，而置换群是一个群$(G,\circ)$，其中$G$是一个置换的集合，$\circ$是置换的复合运算。(置换复合是右结合的!)
...
置换$f$作用于染色$\mathrm{c}$，记为$f\ast\mathrm{c}$。比如染色$(B,B,W,B)$在置换$\left(	\begin{array}{**lr**}1,2,3,4\\ 4,3,2,1\end{array}\right)$作用下会变成$(B,W,B,B)$。
