...
把球顺时针(你喜欢逆时针也行)标号，那么$$G$$一共包含$$n$$个置换，我们按顺时针转过的球个数给这些置换编号，那么容易发现$$\#(f_i)\neq 1$$当且仅当$$i\vert n$$，此时$$\#(f_i)=\frac{n}{i}$$。所以我们枚举一下因数，然后套Pólya定理就好了，复杂度$$O(T\sqrt{n})$$。

-----

[LOJ#6538. 烷基计数 加强版 加强版](https://loj.ac/p/6538)

有根树计数是具有很好的子结构性质的(，相比之下我就对图计数一窍不通，比如下面的烯烃计数)。考虑所有点度数不超过4且根度数不超过3的的有根树的OGF $$A(z)$$，我们枚举根的三个儿子，得到$$A=zA^3+1$$。等等，我们这里并没有考虑儿子无序的问题，然后你就可以开始奆分类，事实上奆分类的本质就是Pólya定理。

要使用Pólya定理，我们的置换群是三个球随便换的置换群 : 
