...
$$
\sum_{i=0}^n(-1)^i\binom{n}{i}\binom{s-it}{n}=s^n
$$

于是直接考虑计算

$$
&\sum_{i=0}^n(-1)^i\binom{n}{i}(s-it-n)^{\underline{m-n}})
$$

于是直接考虑拆掉$$(s-it-m)^{n-m}$$。需要考虑让它和哪个东西拼在一起性质比较好
...
考虑直接解决 两个序列，求有多少区间的$$\max$$相等 这个问题。我们同时在两个序列上跑同一个单调栈，不弹相同的数，然后就可以讨论一下处理掉max相等的段了。这个讨论，ss老师教了我一手，好像很复杂，完全没懂。
...
考虑数点，我们知道对于一个$$k$$，各区间左右端点都是单调的，于是问题变成求查询区间中最后一个右端点和第一个左端点，可以双指针扫出来，但是这个题强制在线，不过我们可以直接预处理前驱后继。
