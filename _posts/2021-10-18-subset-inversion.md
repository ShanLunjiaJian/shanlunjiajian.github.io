...
考虑用总的减去不强连通的，也就是缩点之后得到的DAG点数超过$$1$$的。我们搜一种缩点之后每个SCC的情况进行转移(不搜SCC之间的边)，注意接下来一部分说的都是做一次转移的方法。这个复杂度看起来非常高，不过比起爆搜好的多，它是Bell数$$B(15)=1382958545$$，而直接搜是$$2^{210}>10^{63}$$(还是可以在莉oj 1s冲过去?)，并且Bell数渐进上也确实比$$2^{n(n-1)}$$小的多。
