...
lyndon串定义为字典序小于任何一个自己之外的后缀的串(当然，不包括空)。显然等价于严格小于任何一个自己之外的循环同构的串。所以可以知道lyndon串和重串什么的关系很大。注意全是a的串不是lyndon串，因为它的后缀都比它小。具有非平凡周期的串总不是lyndon串。

lyndon串有很多离奇性质。这里主要提到的它的作用是求一个串的所有run。
...
一个结论是，两个相邻lyndon子串的字典序可以直接比它们的后缀，因为如果$$a$$不是$$b$$的前缀，那么$$a<b\Leftrightarrow ab<b$$。这可以让代码变得更简单。
...
本原重串定义为最小周期恰好是长度一半的串。所以它的真子串都不是重串。

**引理2(三平方定理?)** 如果$$u,v,w$$满足$$uu$$是$$vv$$的前缀，$$vv$$是$$ww$$的前缀，且$$uu$$是本原重串，那么$$\vert u\vert+\vert v\vert\leq\vert w\vert$$。

**证明** 不会。

**结论** 本原重串只有$$O(n\log n)$$个。

**证明** 根据引理2，对于每个左端点，本原重串的长度从小到大排序之后，增长速度至少是fib数列。

**结论** 本质不同的本原重串数量是$$O(n)$$。

**证明** 我们在每个本原重串的最右一次出现处统计它。对于每个左端点，如果有三个本原重串$$uu,vv,ww$$，那么$$uu$$在$$ww$$中出现了两次，所以只有最长的两个可能是最右的出现。
...
为了找到所有本原重串，可以先计算runs，对于每个run $$(l,r,p)$$，$$s_{l,...,r}$$中长$$2p$$的串都是本原重串。根据本原重串的数量，我们知道
...
然后如果允许长$$2kp$$的串，就找到了所有的重串。

-----

使用一个复杂度是调和数的离奇做法，它最早大概来自boi2004 repeats，在09年集训队论文 后缀数组——处理字符串的有力工具 引入，我们不妨称为repeats' trick。我们枚举要找的重串$$uu$$的长度$$2l$$，然后把串按$$l$$大小分块，那么$$u$$一定是一个块前缀和一个块后缀。求出相邻两块的lcp和lcs，然后就得到一个区间的重串，这里还是画一下吧 : 

![img](/2022-05-16-lyndon-word-runs-and-squares/lcp-lcs.png)

这个做法可以很容易地扩展到模糊重串，也就是允许左右两部分有一些不同的重串。oi中一般只会出现汉明距离$$\leq k$$的情况，所以我们使用一个允许汉明距离的lcp，可以转化成$$O(k)$$次普通的lcp并使用sa计算，算完这个把两边卷起来就行了。复杂度$$O(nk\log n)$$。

模糊runs好像不是很能拿lyndon array直接算。

-----

使用main-lorentz算法，它直接分治然后使用z algo辅助求解，好处是不需要sa(所以重串可以出在noip模拟赛!)，并且看起来也很可扩展，坏处是**非常复杂**，**非常复杂**，以及**非常复杂**。
