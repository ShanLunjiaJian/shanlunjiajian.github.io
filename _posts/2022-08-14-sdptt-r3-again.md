...
可以发现那个答案恰好是$$2^{32}-2$$，所以我们知道$$x^{2^{32}-1}\bmod{P}=1$$，所以$$x^k,x^{2^{32}-1-k}$$就是一对逆元。
...
需要讨论圆锥曲线是椭圆，双曲线还是抛物线。如果是双曲线还要特判两条是否相交。
...
考虑平衡说的是啥。对于所有$$i\leq j,k\leq l,j-i=l-k$$，有$$\vert s_j-s_i-s_l+s_j\vert\leq 1$$