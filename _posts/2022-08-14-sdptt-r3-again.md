...
直接做不是很可做。考虑一个性质，子树$$\max$$相同的点是一条链。考虑我们维护这些链，那么换根的时候，旧根到新根的链被翻转，此时链上的点的子树$$\max$$必然变成旧根，所以这个就是链推平，lct维护链，全局开一个BiT即可。这里lct是一个$$\log$$，但是很遗憾每推一段链都要进行一次BiT操作，所以复杂度是俩$$\log$$。
...
考虑dp选$$k$$条可重的边，计算所有点集的斯坦呐树中，有多少个包含这些边。根据二项式定理，这里如果一条边重数是$$c$$，则要带$$\frac{1}{c!}$$的系数。

考虑斯坦呐树包括一条边，也就是在这条边两端都有点，那么也就是说我们选的这些边两端都有点。设$$dp(u,i)$$表示对于$$u$$子树内所有点选/不选的方案，选了$$i$$条边，并且假设$$u$$子树外还选了至少一个点的贡献之和(不计子树外的选点方案数)，在点集的$$\mathrm{lca}$$处统计每个方案。一开始每个点选$$0$$条边的方案是$$2$$，然后转移枚举儿子$$v$$，考虑$$v$$的父边选多少，如果$$v$$子树内没有选过边，那么方案数需要$$-1$$，因为存在一个子树内一个点都没选的方案。发现由于上面说的那个系数，每个点考虑父边选多少的时候是一个卷积，或者说设$$g(u,i)$$表示考虑父边的$$f$$，那么$$g$$是$$f$$和$$\frac{1}{i!}$$的卷积，可以法一法做到$$O(k\log k)$$单次。

考虑现在要在$$u$$处统计答案，因为是$$\mathrm{lca}$$，需要减去选的边都在同一棵子树的方案数。dp部分是树卷积的$$O(nk)$$，也就是每个点的状态数是$$\min(\mathrm{size}(u),k)$$，卷的时候复杂度是每一对兄弟状态数的乘积之和，那么首先三度化不影响复杂度，然后子树大小$$\geq k$$的点不超过$$\frac{n}{k}$$个，的$$O(nk\log k)$$。
...
如果一个数在上一行的这个后缀中没有出现过，那么它可以随便放，否则它有一个不能放的位置。第一个位置只能放一个足够小的数。所以放完我们枚举到的位置之后只有两种数，也就是可以随便放的和有一个位置不能放的。考虑我们枚举放在当前位置这个数是哪一种，问题变成，有$$n$$个数，其中$$k$$个有互不相同的不能放的位置，求重排的方案数。考虑dp，设$$f(n,k)$$表示答案，那么考虑最后一个数随便放的话就是$$f(n,k)=nf(n-1,k)$$
...
考虑什么时候rank是$$n$$，也就是矩阵是满秩的。考虑行列式的定义，

-----

cf1060F Shrinking Tree

考虑这个相当于给每条边标了一个时间，然后每次把一条边加入，把新连通块的颜色置为原来两个连通块的颜色中随机的一个，这样树的形态就不会变了。

考虑无标号无根树个数，或者一棵树上的本质不同连通块个数，看起来都很大啊。

考虑我们枚举一个点，不妨设为$$1$$，那么颜色就变成01了，这就非常阳间。发现$$1$$会进行若干次扩张，每次有一半的概率获胜，所以最后剩下$$1$$的概率只和扩张的次数有关。考虑把$$1$$作为根，那么$$1$$每次扩张，所涉及的边都是连接$$1$$所在的连通块和另一个连通块的最高的点。设$$dp(u,i)$$表示$$1$$需要在$$u$$子树内扩张$$i$$次的方案数(一次扩张计算在被扩张的连通块的最高的点上)，那么枚举$$u$$是不是这个最高的点，如果是则......好像不太行，设$$dp(u,i,0/1)$$表示$$1$$需要在$$u$$子树内扩张$$i$$次的方案数(一次扩张计算在被扩张的连通块的最高的点上)，并且$$1$$占领$$u$$的时候$$u$$不是/是所在连通块最高的点，那么考虑一个儿子$$v$$，如果$$v$$是最高的点，则考虑$$v$$是在$$1$$占领$$u$$之前还是之后和$$u$$合并
...
但是这个假了，因为我们没法保证这个后缀真的是最长的。考虑如果它是一个自动机就好了，所以我们建立sam，然后在上面跑，如果跑到了
...
考虑如何优化。注意到在这个dp中$$len$$就是答案，所以实际上值是$$0/1$$，所以考虑设$$dp(i)$$表示已经处理了$$i$$右边的部分，$$i$$作为第一个串左端点的情况下，最大的$$len$$能是多少。可以注意到如果存在$$j$$满足$$i<j,dp(i)<dp(j)$$，那么$$dp(i)$$啥用没有，因为从它转移了也不如直接跳到$$dp(j)$$。所以我们只要判定是否能有$$dp(i)=\max dp(j)$$或$$dp(i)=\max dp(j)+1$$，显然不可能有$$dp(i)>\max dp(j)+1$$，这也就是找到$$\max dp(j)$$
...
另一个做法是直接不设$$j$$，而是枚举上一个这个在哪，不过我感觉这个为啥是对的啊。
...
考虑爆力，直接从每个关键点连到它为中心半径$$k$$的邻域，点分之后拆一拆即可，看起来复杂度是$$O(n\log n\alpha(n))$$。

考虑一个简单做法。先考虑如果查询的都是关键点怎么做。

这个是半径为$$\frac{k}{2}$$的邻域的相交图。

考虑合并必然可以只在关键点虚树的边上发生。
...
考虑边数只有$$n^{\lg 3}$$，所以爆力即可。复杂度$$O(n^{\lg 2}\alpha(n))$$。
...
考虑分块，每$$k$$个一块。考虑我们把一块都塞进去，然后把另一块塞进去，然后清空，然后把第一块倒着塞进去，然后把第二块塞进去，我们就知道第二块中每个数，在第一块中有没有和它相同的数了。如果一个数满足在每一块里都没有和它相同的数，那么
...
考虑我们不可能选两个电阻使得它们的阻值都非$$0$$，并且一条电流同时经过它们两个，否则把阻值都调整到其中一个上必然不劣。
...
考虑一个序列必然存在一个极大好区间划分，那么设答案是$$F$$，排列总数是$$A$$，我们知道$$F=A-\frac{F^2}{1-F}+z$$，多项式开根即可。

B. algebra

给$$a,m,n$$，求每次把$$a$$加上$$a$$十进制下最大的数位再膜$$m$$，$$n$$次操作后得到的数。$$10^4$$组询问，$$n,m\leq 10^{18}$$。

发现这个取膜相当于没有，因为每次膜完了必然在$$0,...,8$$之中。

考虑怎么快速求不取膜操作$$n$$次之后会变成啥。考虑我们找到最大的位，看它到什么时候会变化
