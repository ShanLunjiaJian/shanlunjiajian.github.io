...
考虑把所有互素的对画到平面上，那么相当于拿$$xy=k$$去切这个东西，那么我们知道答案必然在右上单调栈上。从大往小扫$$i$$，我们知道如果$$(i,j>i)$$合法，那么$$<j$$的都没用了，于是考虑对$$j$$维护一个栈，问题是计算栈中有多少数和$$i$$互素。

考虑莫反。结束了，跑一次的复杂度是$$O(n\log n)$$，然后枚举$$\gcd$$就是跑总长$$n\log n$$的，所以总复杂度$$O(n\log^2 n)$$。
