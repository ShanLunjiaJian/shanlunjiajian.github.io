...
先考虑$$\pi(m)=\mathrm{lcm}\limits_{p^k\parallel m}\pi(p^k)$$，而显然有$$\pi(p^k)\leq p^{k-1}\pi(p)$$，所以我们尝试考虑$$\pi(p)$$。只考虑$$p>2$$的情况。
...
如果$$\alpha,\beta$$在膜$$p$$下不存在，注意到由于$$\alpha=\frac{a}{2}+\frac{\sqrt{a^2+4b}}{2}$$，而经典结论指出$$(x+a)^n\equiv x^n+a\pmod{n}$$，于是你发现$$\alpha^p\equiv\frac{a+(a^2+4b)^\frac{p-1}{2}\sqrt{a^2+4b}}{2}$$。由于$$a^2+4b$$不是二次剩余(否则刚才讨论过了)，$$(a^2+4b)^\frac{p-1}{2}=-1$$。这说明$$\alpha^p\equiv\beta$$，而根据韦达定理又有$$\alpha\beta=-b$$，于是$$\alpha^{p+1}\equiv -b$$，于是$$\alpha^{2(p+1)\mathrm{ord})p(b)}\equiv 1$$，也就是$$\alpha$$的阶是$$2(p+1)\mathrm{ord}_p(b)$$的因数。同理$$\beta$$的阶也是$$2(p+1)\mathrm{ord}_p(b)$$的因数，于是循环节，也就是两者阶的$$\mathrm{lcm}$$，还是$$2(p+1)\mathrm{ord}_p(b)$$的因数。
