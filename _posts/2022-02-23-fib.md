...
**证明** 先考虑$$\pi(m)=\mathop{\mathrm{lcm}}\limits_{p^k\parallel m}\pi(p^k)$$，而显然有$$\pi(p^k)\leq p^{k-1}\pi(p)$$，所以我们尝试考虑$$\pi(p)$$。只考虑$$p>2$$的情况，因为$$p=2$$的时候对任意广义fib数列显然都很好。
...
考虑广义fib数的特征多项式$$q(z)=z^2-az-b=(z-\alpha)(z-\beta)$$，其中$$\alpha,\beta=\displaystyle\frac{a\pm\sqrt{a^2+4b}}{2}$$。
 ...
 - 如果$$\alpha=\beta$$，那么有$$f_n=(A+Bn)\alpha^n$$，其中$$A,B$$是另一些常数，并且$$\alpha$$必然存在。这种情况出现，当且仅当$$p\mid a^2+4b$$。$$A+Bn$$的阶是$$p$$(由于$$p$$是素数，这玩意是紧的)，$$\alpha^n$$则需要观察一下。因为$$a^2+4b\equiv 0$$，所以$$\displaystyle\left(\frac{a}{2}\right)^2\equiv -b$$，于是$$\operatorname{ord}_p(a)\leq 2\operatorname{ord}_p(-b)$$。一共是$$2p\operatorname{ord}_p(-b)$$。
