...
先考虑$$\pi(m)=\mathop{\mathrm{lcm}}\limits_{p^k\parallel m}\pi(p^k)$$，而显然有$$\pi(p^k)\leq p^{k-1}\pi(p)$$，所以我们尝试考虑$$\pi(p)$$。只考虑$$p>2$$的情况，因为$$p=2$$的时候对任意广义fib数列显然都很好。
...
如果$$\alpha,\beta$$在膜$$p$$下不存在，注意到由于$$\displaystyle\alpha=\frac{a}{2}+\frac{\sqrt{a^2+4b}}{2}$$，而经典结论指出$$(x+a)^n\equiv x^n+a\pmod{n}$$，于是你发现$$\displaystyle\alpha^p\equiv\frac{a+(a^2+4b)^\frac{p-1}{2}\sqrt{a^2+4b}}{2}$$。由于$$a^2+4b$$不是二次剩余(否则刚才讨论过了)，$$\displaystyle(a^2+4b)^\frac{p-1}{2}=-1$$。这说明$$\alpha^p\equiv\beta$$，而根据韦达定理又有$$\alpha\beta=-b$$，于是$$\alpha^{p+1}\equiv -b$$，于是$$\alpha^{(p+1)\mathrm{ord}_p(-b)}\equiv 1$$，也就是$$\alpha$$的阶是$$(p+1)\mathrm{ord}_p(-b)$$的因数。同理$$\beta$$的阶也是$$(p+1)\mathrm{ord}_p(-b)$$的因数，于是循环节还是$$(p+1)\mathrm{ord}_p(-b)$$的因数。

剩下的部分是，如果$$\alpha=\beta$$。这种情况出现，当且仅当$$p\mid a^2+4b$$。此时$$f_n=(A+Bn)\alpha^n$$，其中$$A,B$$是另一些常数，并且$$\alpha$$必然存在。$$A+Bn$$的阶是$$p$$(由于$$p$$是素数，这玩意是紧的)，$$\alpha^n$$则需要观察一下。因为$$a^2+4b\equiv 0$$，所以$$\displaystyle\left(\frac{a}{2}\right^2\equiv -b$$，于是$$\mathrm{ord}_p(a)\leq 2\mathrm{ord}_p(-b)$$。一共是$$2p\mathrm{ord}_p(-b)$$。

-----

**结论** fib数列$$f_n=f_{n-1}+f_{n-2}$$膜$$m$$的循环节，设为$$\pi(m)$$，不超过$$6m$$。

**证明** 注意到$$\mathrm{ord}_p(-b)=2$$。
