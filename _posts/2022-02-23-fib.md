...
考虑任意常系数齐次线性递推定义的数列，根据有理展开定理，它的解必然是$$f_n=\sum A_i(n)\alpha_i^n$$，并且感觉一下，每个$$A_i(n)\alpha_i^n$$膜$$p$$的循环节都是$$\pi(p)$$的因数，否则必然是有若干项拼在一起抵消了，此时我们可以把它们合并成一项这样的。
...
要寻找任意广义fib数列的循环节，我们可以做到$$O(\sqrt{m})$$。上一个pr找到所有素因数，然后对于一个素因数$$p^k$$，需要对一个$$O(p^2)$$的数跑pr，然后对每个因数做一次矩阵快速幂，然后对每个素因数取$$\mathrm{lcm}$$。总复杂度看起来是$$O(\sqrt{m}+d(m)\log m)$$，后者比较低阶可以扔了。

如果是膜素数$$p$$的循环节，并且$$\operatorname{ord}_p(-b)$$比$$p$$低阶，那么复杂度降为$$O(\sqrt[4]{p\operatorname{ord}_p(-b)})$$。但是在四次根号的数据范围内，$$d(p)\log p$$貌似是个难以忽略的常数，比如$$d(10^{18})>10^5$$。

如果$$m$$很小，简单做法是floyd找环，期望复杂度是$$O(\sqrt{m})$$。或者你也可以把pr换成爆力，复杂度是$$O(\frac{\sqrt{m}}{\log m})$$，看起来要更棒一些。很多题其实只需要后者。
