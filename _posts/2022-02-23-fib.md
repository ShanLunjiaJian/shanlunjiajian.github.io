...
。你可以在具体数学上看到这个。这个东西居然真的有用![/jy](/img/qqemoji/0.gif)，一会我们真的会用到它。
...
考虑任意常系数齐次线性递推定义的数列，根据有理展开定理，它的解必然是$$f_n=\sum A_i(n)\alpha_i^n$$，并且感觉一下，每个$$A_i(n)\alpha_i^n$$膜$$p$$的循环节都是$$\pi(p)$$的因数。
...
据我校mo爷 fanzor (你可能找不到这个id?他的谷也许是zhaozr79)说，这个引理0.5可以换成升幂定理，但是我也不懂。
