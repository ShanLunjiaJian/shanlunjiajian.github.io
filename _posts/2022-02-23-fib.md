...
如果$$\alpha,\beta$$在膜$$p$$下不存在，注意到由于$$\displaystyle\alpha=\frac{a}{2}+\frac{\sqrt{a^2+4b}}{2}$$，而经典结论指出$$(x+a)^n\equiv x^n+a\pmod{n}$$，于是你发现$$\displaystyle\alpha^p\equiv\frac{a+(a^2+4b)^\frac{p-1}{2}\sqrt{a^2+4b}}{2}$$。由于$$a^2+4b$$不是二次剩余(否则刚才讨论过了)，$$\displaystyle(a^2+4b)^\frac{p-1}{2}=-1$$。这说明$$\alpha^p\equiv\beta$$，而根据韦达定理又有$$\alpha\beta=-b$$，于是$$\alpha^{p+1}\equiv -b$$，于是$$\alpha^{2(p+1)\mathrm{ord})p(b)}\equiv 1$$，也就是$$\alpha$$的阶是$$2(p+1)\mathrm{ord}_p(b)$$的因数。同理$$\beta$$的阶也是$$2(p+1)\mathrm{ord}_p(b)$$的因数，于是循环节，也就是两者阶的$$\mathrm{lcm}$$，还是$$2(p+1)\mathrm{ord}_p(b)$$的因数。
